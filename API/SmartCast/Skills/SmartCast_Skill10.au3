#include-once

; Skill ID: 29 - $GC_I_SKILL_ID_BLACKOUT
Func CanUse_Blackout()
	Return True
EndFunc

Func BestTarget_Blackout($aAggroRange)
	Return 0
EndFunc

; Skill ID: 154 - $GC_I_SKILL_ID_PLAGUE_TOUCH
Func CanUse_PlagueTouch()
	Return True
EndFunc

Func BestTarget_PlagueTouch($aAggroRange)
	Return 0
EndFunc

; Skill ID: 155 - $GC_I_SKILL_ID_VILE_TOUCH
Func CanUse_VileTouch()
	Return True
EndFunc

Func BestTarget_VileTouch($aAggroRange)
	Return 0
EndFunc

; Skill ID: 156 - $GC_I_SKILL_ID_VAMPIRIC_TOUCH
Func CanUse_VampiricTouch()
	Return True
EndFunc

Func BestTarget_VampiricTouch($aAggroRange)
	Return 0
EndFunc

; Skill ID: 158 - $GC_I_SKILL_ID_TOUCH_OF_AGONY
Func CanUse_TouchOfAgony()
	Return True
EndFunc

Func BestTarget_TouchOfAgony($aAggroRange)
	Return 0
EndFunc

; Skill ID: 231 - $GC_I_SKILL_ID_SHOCK
Func CanUse_Shock()
	Return True
EndFunc

Func BestTarget_Shock($aAggroRange)
	Return 0
EndFunc

; Skill ID: 232 - $GC_I_SKILL_ID_LIGHTNING_TOUCH
Func CanUse_LightningTouch()
	Return True
EndFunc

Func BestTarget_LightningTouch($aAggroRange)
	Return 0
EndFunc

; Skill ID: 300 - $GC_I_SKILL_ID_CONTEMPLATION_OF_PURITY
Func CanUse_ContemplationOfPurity()
	Return True
EndFunc

Func BestTarget_ContemplationOfPurity($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 312 - $GC_I_SKILL_ID_HOLY_STRIKE
Func CanUse_HolyStrike()
	Return True
EndFunc

Func BestTarget_HolyStrike($aAggroRange)
	Return 0
EndFunc

; Skill ID: 389 - $GC_I_SKILL_ID_FLOURISH
Func CanUse_Flourish()
	Return True
EndFunc

Func BestTarget_Flourish($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 411 - $GC_I_SKILL_ID_CHARM_ANIMAL
Func CanUse_CharmAnimal()
	Return True
EndFunc

Func BestTarget_CharmAnimal($aAggroRange)
	Return 0
EndFunc

; Skill ID: 422 - $GC_I_SKILL_ID_REVIVE_ANIMAL
Func CanUse_ReviveAnimal()
	Return True
EndFunc

Func BestTarget_ReviveAnimal($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 424 - $GC_I_SKILL_ID_THROW_DIRT
Func CanUse_ThrowDirt()
	Return True
EndFunc

Func BestTarget_ThrowDirt($aAggroRange)
	Return 0
EndFunc

; Skill ID: 436 - $GC_I_SKILL_ID_COMFORT_ANIMAL
Func CanUse_ComfortAnimal()
	Return True
EndFunc

Func BestTarget_ComfortAnimal($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 446 - $GC_I_SKILL_ID_TROLL_UNGUENT
Func CanUse_TrollUnguent()
	Return True
EndFunc

Func BestTarget_TrollUnguent($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 502 - $GC_I_SKILL_ID_RESURRECT_PARTY
Func CanUse_ResurrectParty()
	Return True
EndFunc

Func BestTarget_ResurrectParty($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 510 - $GC_I_SKILL_ID_DEAFENING_ROAR
Func CanUse_DeafeningRoar()
	Return True
EndFunc

Func BestTarget_DeafeningRoar($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 513 - $GC_I_SKILL_ID_CHARM_ANIMAL_MONSTER_SKILL
; Skill ID: 514 - $GC_I_SKILL_ID_BREAKING_CHARM
Func CanUse_BreakingCharm()
	Return True
EndFunc

Func BestTarget_BreakingCharm($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 516 - $GC_I_SKILL_ID_CLAIM_RESOURCE1
Func CanUse_ClaimResource1()
	Return True
EndFunc

Func BestTarget_ClaimResource1($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 517 - $GC_I_SKILL_ID_CLAIM_RESOURCE2
Func CanUse_ClaimResource2()
	Return True
EndFunc

Func BestTarget_ClaimResource2($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 518 - $GC_I_SKILL_ID_CLAIM_RESOURCE3
Func CanUse_ClaimResource3()
	Return True
EndFunc

Func BestTarget_ClaimResource3($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 519 - $GC_I_SKILL_ID_CLAIM_RESOURCE4
Func CanUse_ClaimResource4()
	Return True
EndFunc

Func BestTarget_ClaimResource4($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 520 - $GC_I_SKILL_ID_CLAIM_RESOURCE5
Func CanUse_ClaimResource5()
	Return True
EndFunc

Func BestTarget_ClaimResource5($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 521 - $GC_I_SKILL_ID_CLAIM_RESOURCE6
Func CanUse_ClaimResource6()
	Return True
EndFunc

Func BestTarget_ClaimResource6($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 522 - $GC_I_SKILL_ID_CLAIM_RESOURCE7
Func CanUse_ClaimResource7()
	Return True
EndFunc

Func BestTarget_ClaimResource7($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 523 - $GC_I_SKILL_ID_CLAIM_RESOURCE8
Func CanUse_ClaimResource8()
	Return True
EndFunc

Func BestTarget_ClaimResource8($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 526 - $GC_I_SKILL_ID_CLAIM_RESOURCE9
Func CanUse_ClaimResource9()
	Return True
EndFunc

Func BestTarget_ClaimResource9($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 527 - $GC_I_SKILL_ID_CLAIM_RESOURCE10
Func CanUse_ClaimResource10()
	Return True
EndFunc

Func BestTarget_ClaimResource10($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 536 - $GC_I_SKILL_ID_CRYSTAL_HIBERNATION
Func CanUse_CrystalHibernation()
	Return True
EndFunc

Func BestTarget_CrystalHibernation($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 541 - $GC_I_SKILL_ID_LIFE_VORTEX
Func CanUse_LifeVortex()
	Return True
EndFunc

Func BestTarget_LifeVortex($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 546 - $GC_I_SKILL_ID_SPECTRAL_AGONY
Func CanUse_SpectralAgony()
	Return True
EndFunc

Func BestTarget_SpectralAgony($aAggroRange)
	Return 0
EndFunc

; Skill ID: 565 - $GC_I_SKILL_ID_CLAIM_RESOURCE
Func CanUse_ClaimResource()
	Return True
EndFunc

Func BestTarget_ClaimResource($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 573 - $GC_I_SKILL_ID_TELEPORT_PLAYERS
Func CanUse_TeleportPlayers()
	Return True
EndFunc

Func BestTarget_TeleportPlayers($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 588 - $GC_I_SKILL_ID_CHARM_ANIMAL_MONSTER
; Skill ID: 590 - $GC_I_SKILL_ID_KNOCK
Func CanUse_Knock()
	Return True
EndFunc

Func BestTarget_Knock($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 786 - $GC_I_SKILL_ID_IRON_PALM
Func CanUse_IronPalm()
	Return True
EndFunc

Func BestTarget_IronPalm($aAggroRange)
	Return 0
EndFunc

; Skill ID: 810 - $GC_I_SKILL_ID_PROTECTORS_DEFENSE
Func CanUse_ProtectorsDefense()
	Return True
EndFunc

Func BestTarget_ProtectorsDefense($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 990 - $GC_I_SKILL_ID_EXPUNGE_ENCHANTMENTS
Func CanUse_ExpungeEnchantments()
	Return True
EndFunc

Func BestTarget_ExpungeEnchantments($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1033 - $GC_I_SKILL_ID_IMPALE
Func CanUse_Impale()
	Return True
EndFunc

Func BestTarget_Impale($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1039 - $GC_I_SKILL_ID_STAR_STRIKE
Func CanUse_StarStrike()
	Return True
EndFunc

Func BestTarget_StarStrike($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1045 - $GC_I_SKILL_ID_PALM_STRIKE
Func CanUse_PalmStrike()
	Return True
EndFunc

Func BestTarget_PalmStrike($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1072 - $GC_I_SKILL_ID_STAR_SERVANT
Func CanUse_StarServant()
	Return True
EndFunc

Func BestTarget_StarServant($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1077 - $GC_I_SKILL_ID_VAMPIRIC_BITE
Func CanUse_VampiricBite()
	Return True
EndFunc

Func BestTarget_VampiricBite($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1078 - $GC_I_SKILL_ID_WALLOWS_BITE
Func CanUse_WallowsBite()
	Return True
EndFunc

Func BestTarget_WallowsBite($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1079 - $GC_I_SKILL_ID_ENFEEBLING_TOUCH
Func CanUse_EnfeeblingTouch()
	Return True
EndFunc

Func BestTarget_EnfeeblingTouch($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1100 - $GC_I_SKILL_ID_CELESTIAL_STORM
Func CanUse_CelestialStorm()
	Return True
EndFunc

Func BestTarget_CelestialStorm($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1124 - $GC_I_SKILL_ID_STAR_SHINE
Func CanUse_StarShine()
	Return True
EndFunc

Func BestTarget_StarShine($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1131 - $GC_I_SKILL_ID_STONESOUL_STRIKE
Func CanUse_StonesoulStrike()
	Return True
EndFunc

Func BestTarget_StonesoulStrike($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1140 - $GC_I_SKILL_ID_STORM_OF_SWORDS
Func CanUse_StormOfSwords()
	Return True
EndFunc

Func BestTarget_StormOfSwords($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1146 - $GC_I_SKILL_ID_SHOVE
Func CanUse_Shove()
	Return True
EndFunc

Func BestTarget_Shove($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1147 - $GC_I_SKILL_ID_BASE_DEFENSE
Func CanUse_BaseDefense()
	Return True
EndFunc

Func BestTarget_BaseDefense($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1148 - $GC_I_SKILL_ID_CARRIER_DEFENSE
Func CanUse_CarrierDefense()
	Return True
EndFunc

Func BestTarget_CarrierDefense($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1149 - $GC_I_SKILL_ID_THE_CHALICE_OF_CORRUPTION
Func CanUse_TheChaliceOfCorruption()
	Return True
EndFunc

Func BestTarget_TheChaliceOfCorruption($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1155 - $GC_I_SKILL_ID_JUGGERNAUT_TOSS
Func CanUse_JuggernautToss()
	Return True
EndFunc

Func BestTarget_JuggernautToss($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1178 - $GC_I_SKILL_ID_LAST_STAND
Func CanUse_LastStand()
	Return True
EndFunc

Func BestTarget_LastStand($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1185 - $GC_I_SKILL_ID_CONSTRUCT_POSSESSION
Func CanUse_ConstructPossession()
	Return True
EndFunc

Func BestTarget_ConstructPossession($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1186 - $GC_I_SKILL_ID_SIEGE_TURTLE_ATTACK_THE_ETERNAL_GROVE
Func CanUse_SiegeTurtleAttackTheEternalGrove()
	Return True
EndFunc

Func BestTarget_SiegeTurtleAttackTheEternalGrove($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1187 - $GC_I_SKILL_ID_SIEGE_TURTLE_ATTACK_FORT_ASPENWOOD
Func CanUse_SiegeTurtleAttackFortAspenwood()
	Return True
EndFunc

Func BestTarget_SiegeTurtleAttackFortAspenwood($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1188 - $GC_I_SKILL_ID_SIEGE_TURTLE_ATTACK_GYALA_HATCHERY
Func CanUse_SiegeTurtleAttackGyalaHatchery()
	Return True
EndFunc

Func BestTarget_SiegeTurtleAttackGyalaHatchery($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1195 - $GC_I_SKILL_ID_HEAL_AS_ONE
Func CanUse_HealAsOne()
	Return True
EndFunc

Func BestTarget_HealAsOne($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1207 - $GC_I_SKILL_ID_CELESTIAL_STANCE
Func CanUse_CelestialStance()
	Return True
EndFunc

Func BestTarget_CelestialStance($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1232 - $GC_I_SKILL_ID_ARMOR_OF_UNFEELING
Func CanUse_ArmorOfUnfeeling()
	Return True
EndFunc

Func BestTarget_ArmorOfUnfeeling($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1241 - $GC_I_SKILL_ID_CELESTIAL_SUMMONING
Func CanUse_CelestialSummoning()
	Return True
EndFunc

Func BestTarget_CelestialSummoning($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1246 - $GC_I_SKILL_ID_ANCESTORS_RAGE
Func CanUse_AncestorsRage()
	Return True
EndFunc

Func BestTarget_AncestorsRage($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1273 - $GC_I_SKILL_ID_IMPOSSIBLE_ODDS
Func CanUse_ImpossibleOdds()
	Return True
EndFunc

Func BestTarget_ImpossibleOdds($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1276 - $GC_I_SKILL_ID_MEDITATION_OF_THE_REAPER1
Func CanUse_MeditationOfTheReaper1()
	Return True
EndFunc

Func BestTarget_MeditationOfTheReaper1($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1307 - $GC_I_SKILL_ID_FORESTS_BINDING
Func CanUse_ForestsBinding()
	Return True
EndFunc

Func BestTarget_ForestsBinding($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1308 - $GC_I_SKILL_ID_EXPLODING_SPORES
Func CanUse_ExplodingSpores()
	Return True
EndFunc

Func BestTarget_ExplodingSpores($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1315 - $GC_I_SKILL_ID_RAGE_OF_THE_SEA
Func CanUse_RageOfTheSea()
	Return True
EndFunc

Func BestTarget_RageOfTheSea($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1316 - $GC_I_SKILL_ID_MEDITATION_OF_THE_REAPER2
Func CanUse_MeditationOfTheReaper2()
	Return True
EndFunc

Func BestTarget_MeditationOfTheReaper2($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1317 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 1324 - $GC_I_SKILL_ID_TORMENT_SLASH
Func CanUse_TormentSlash()
	Return True
EndFunc

Func BestTarget_TormentSlash($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1326 - $GC_I_SKILL_ID_TRADE_WINDS
Func CanUse_TradeWinds()
	Return True
EndFunc

Func BestTarget_TradeWinds($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1327 - $GC_I_SKILL_ID_DRAGON_BLAST
Func CanUse_DragonBlast()
	Return True
EndFunc

Func BestTarget_DragonBlast($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1328 - $GC_I_SKILL_ID_IMPERIAL_MAJESTY
Func CanUse_ImperialMajesty()
	Return True
EndFunc

Func BestTarget_ImperialMajesty($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1339 - $GC_I_SKILL_ID_SYMBOLS_OF_INSPIRATION
Func CanUse_SymbolsOfInspiration()
	Return True
EndFunc

Func BestTarget_SymbolsOfInspiration($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1386 - $GC_I_SKILL_ID_SENTRY_TRAP_SKILL
Func CanUse_SentryTrapSkill()
	Return True
EndFunc

Func BestTarget_SentryTrapSkill($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1387 - $GC_I_SKILL_ID_CALTROPS_MONSTER
Func CanUse_CaltropsMonster()
	Return True
EndFunc

Func BestTarget_CaltropsMonster($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1388 - $GC_I_SKILL_ID_SACRED_BRANCH
Func CanUse_SacredBranch()
	Return True
EndFunc

Func BestTarget_SacredBranch($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1389 - $GC_I_SKILL_ID_LIGHT_OF_SEBORHIN
Func CanUse_LightOfSeborhin()
	Return True
EndFunc

Func BestTarget_LightOfSeborhin($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1406 - $GC_I_SKILL_ID_HEADBUTT
Func CanUse_Headbutt()
	Return True
EndFunc

Func BestTarget_Headbutt($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1407 - $GC_I_SKILL_ID_LIONS_COMFORT
Func CanUse_LionsComfort()
	Return True
EndFunc

Func BestTarget_LionsComfort($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1418 - $GC_I_SKILL_ID_DISARM_TRAP
Func CanUse_DisarmTrap()
	Return True
EndFunc

Func BestTarget_DisarmTrap($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1422 - $GC_I_SKILL_ID_CREATE_LIGHT_OF_SEBORHIN
Func CanUse_CreateLightOfSeborhin()
	Return True
EndFunc

Func BestTarget_CreateLightOfSeborhin($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1423 - $GC_I_SKILL_ID_UNLOCK_CELL
Func CanUse_UnlockCell()
	Return True
EndFunc

Func BestTarget_UnlockCell($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1438 - $GC_I_SKILL_ID_JUNUNDU_FEAST
Func CanUse_JununduFeast()
	Return True
EndFunc

Func BestTarget_JununduFeast($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1439 - $GC_I_SKILL_ID_JUNUNDU_STRIKE
Func CanUse_JununduStrike()
	Return True
EndFunc

Func BestTarget_JununduStrike($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1440 - $GC_I_SKILL_ID_JUNUNDU_SMASH
Func CanUse_JununduSmash()
	Return True
EndFunc

Func BestTarget_JununduSmash($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1441 - $GC_I_SKILL_ID_JUNUNDU_SIEGE1
Func CanUse_JununduSiege1()
	Return True
EndFunc

Func BestTarget_JununduSiege1($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1443 - $GC_I_SKILL_ID_LEAVE_JUNUNDU
Func CanUse_LeaveJunundu()
	Return True
EndFunc

Func BestTarget_LeaveJunundu($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1445 - $GC_I_SKILL_ID_SIGNAL_FLARE
Func CanUse_SignalFlare()
	Return True
EndFunc

Func BestTarget_SignalFlare($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1446 - $GC_I_SKILL_ID_THE_ELIXIR_OF_STRENGTH
Func CanUse_TheElixirOfStrength()
	Return True
EndFunc

Func BestTarget_TheElixirOfStrength($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1447 - $GC_I_SKILL_ID_EHZAH_FROM_ABOVE
Func CanUse_EhzahFromAbove()
	Return True
EndFunc

Func BestTarget_EhzahFromAbove($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1454 - $GC_I_SKILL_ID_CALL_TO_THE_TORMENT
Func CanUse_CallToTheTorment()
	Return True
EndFunc

Func BestTarget_CallToTheTorment($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1455 - $GC_I_SKILL_ID_COMMAND_OF_TORMENT
Func CanUse_CommandOfTorment()
	Return True
EndFunc

Func BestTarget_CommandOfTorment($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1456 - $GC_I_SKILL_ID_ABADDONS_FAVOR
Func CanUse_AbaddonsFavor()
	Return True
EndFunc

Func BestTarget_AbaddonsFavor($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1471 - $GC_I_SKILL_ID_SCAVENGERS_FOCUS
Func CanUse_ScavengersFocus()
	Return True
EndFunc

Func BestTarget_ScavengersFocus($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1573 - $GC_I_SKILL_ID_AWE
Func CanUse_Awe()
	Return True
EndFunc

Func BestTarget_Awe($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1583 - $GC_I_SKILL_ID_LEADERS_ZEAL
Func CanUse_LeadersZeal()
	Return True
EndFunc

Func BestTarget_LeadersZeal($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1584 - $GC_I_SKILL_ID_LEADERS_COMFORT
Func CanUse_LeadersComfort()
	Return True
EndFunc

Func BestTarget_LeadersComfort($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1586 - $GC_I_SKILL_ID_ANGELIC_PROTECTION
Func CanUse_AngelicProtection()
	Return True
EndFunc

Func BestTarget_AngelicProtection($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1587 - $GC_I_SKILL_ID_ANGELIC_BOND
Func CanUse_AngelicBond()
	Return True
EndFunc

Func BestTarget_AngelicBond($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1609 - $GC_I_SKILL_ID_SCEPTER_OF_ETHER
Func CanUse_ScepterOfEther()
	Return True
EndFunc

Func BestTarget_ScepterOfEther($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1616 - $GC_I_SKILL_ID_QUEEN_HEAL
Func CanUse_QueenHeal()
	Return True
EndFunc

Func BestTarget_QueenHeal($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1619 - $GC_I_SKILL_ID_QUEEN_BITE
Func CanUse_QueenBite()
	Return True
EndFunc

Func BestTarget_QueenBite($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1620 - $GC_I_SKILL_ID_QUEEN_THUMP
Func CanUse_QueenThump()
	Return True
EndFunc

Func BestTarget_QueenThump($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1621 - $GC_I_SKILL_ID_QUEEN_SIEGE
Func CanUse_QueenSiege()
	Return True
EndFunc

Func BestTarget_QueenSiege($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1643 - $GC_I_SKILL_ID_ASSAULT_ENCHANTMENTS
Func CanUse_AssaultEnchantments()
	Return True
EndFunc

Func BestTarget_AssaultEnchantments($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1644 - $GC_I_SKILL_ID_WASTRELS_COLLAPSE
Func CanUse_WastrelsCollapse()
	Return True
EndFunc

Func BestTarget_WastrelsCollapse($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1645 - $GC_I_SKILL_ID_LIFT_ENCHANTMENT
Func CanUse_LiftEnchantment()
	Return True
EndFunc

Func BestTarget_LiftEnchantment($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1706 - $GC_I_SKILL_ID_CORRUPT_POWER
Func CanUse_CorruptPower()
	Return True
EndFunc

Func BestTarget_CorruptPower($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1708 - $GC_I_SKILL_ID_GAZE_OF_MOAVUKAAL
Func CanUse_GazeOfMoavukaal()
	Return True
EndFunc

Func BestTarget_GazeOfMoavukaal($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1711 - $GC_I_SKILL_ID_THE_APOCRYPHA_IS_CHANGING_TO_ANOTHER_FORM
Func CanUse_TheApocryphaIsChangingToAnotherForm()
	Return True
EndFunc

Func BestTarget_TheApocryphaIsChangingToAnotherForm($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1715 - $GC_I_SKILL_ID_REFORM_CARVINGS
Func CanUse_ReformCarvings()
	Return True
EndFunc

Func BestTarget_ReformCarvings($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1718 - $GC_I_SKILL_ID_SOUL_TORTURE
Func CanUse_SoulTorture()
	Return True
EndFunc

Func BestTarget_SoulTorture($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1814 - $GC_I_SKILL_ID_LIGHTBRINGERS_GAZE
Func CanUse_LightbringersGaze()
	Return True
EndFunc

Func BestTarget_LightbringersGaze($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1818 - $GC_I_SKILL_ID_MADDENED_STRIKE
Func CanUse_MaddenedStrike()
	Return True
EndFunc

Func BestTarget_MaddenedStrike($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1855 - $GC_I_SKILL_ID_KOURNAN_SIEGE
Func CanUse_KournanSiege()
	Return True
EndFunc

Func BestTarget_KournanSiege($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1861 - $GC_I_SKILL_ID_CHOKING_BREATH
Func CanUse_ChokingBreath()
	Return True
EndFunc

Func BestTarget_ChokingBreath($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1862 - $GC_I_SKILL_ID_JUNUNDU_BITE
Func CanUse_JununduBite()
	Return True
EndFunc

Func BestTarget_JununduBite($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1863 - $GC_I_SKILL_ID_BLINDING_BREATH
Func CanUse_BlindingBreath()
	Return True
EndFunc

Func BestTarget_BlindingBreath($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1864 - $GC_I_SKILL_ID_BURNING_BREATH
Func CanUse_BurningBreath()
	Return True
EndFunc

Func BestTarget_BurningBreath($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1865 - $GC_I_SKILL_ID_JUNUNDU_WAIL
Func CanUse_JununduWail()
	If CachedAgent_HasEffect($GC_I_SKILL_ID_CURSE_OF_DHUUM) Or CachedAgent_HasEffect($GC_I_SKILL_ID_FROZEN_SOIL) Then Return False
	Return True
EndFunc

Func BestTarget_JununduWail($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1868 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 1869 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 1870 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 1874 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 1875 - $GC_I_SKILL_ID_WORDS_OF_MADNESS
Func CanUse_WordsOfMadness()
	Return True
EndFunc

Func BestTarget_WordsOfMadness($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1876 - $GC_I_SKILL_ID_UNKNOWN_JUNUNDU_ABILITY
Func CanUse_UnknownJununduAbility()
	Return True
EndFunc

Func BestTarget_UnknownJununduAbility($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1880 - $GC_I_SKILL_ID_TORMENT_SLASH_SMOTHERING_TENDRILS
Func CanUse_TormentSlashSmotheringTendrils()
	Return True
EndFunc

Func BestTarget_TormentSlashSmotheringTendrils($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1884 - $GC_I_SKILL_ID_CONSUME_TORMENT
Func CanUse_ConsumeTorment()
	Return True
EndFunc

Func BestTarget_ConsumeTorment($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1886 - $GC_I_SKILL_ID_SUMMONING_SHADOWS
Func CanUse_SummoningShadows()
	Return True
EndFunc

Func BestTarget_SummoningShadows($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1894 - $GC_I_SKILL_ID_TOUCH_OF_AAAAARRRRRRGGGHHH
Func CanUse_TouchOfAaaaarrrrrrggghhh()
	Return True
EndFunc

Func BestTarget_TouchOfAaaaarrrrrrggghhh($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1900 - $GC_I_SKILL_ID_SIDE_STEP
Func CanUse_SideStep()
	Return True
EndFunc

Func BestTarget_SideStep($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1906 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 1907 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 1908 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 1909 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 1910 - $GC_I_SKILL_ID_CHARM_ANIMAL_WHITE_MANTLE
Func CanUse_CharmAnimalWhiteMantle()
	Return True
EndFunc

Func BestTarget_CharmAnimalWhiteMantle($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1913 - $GC_I_SKILL_ID_CLAIM_RESOURCE_HEROES_ASCENT
Func CanUse_ClaimResourceHeroesAscent()
	Return True
EndFunc

Func BestTarget_ClaimResourceHeroesAscent($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1926 - $GC_I_SKILL_ID_LUNAR_BLESSING
Func CanUse_LunarBlessing()
	Return True
EndFunc

Func BestTarget_LunarBlessing($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1927 - $GC_I_SKILL_ID_LUCKY_AURA
Func CanUse_LuckyAura()
	Return True
EndFunc

Func BestTarget_LuckyAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1928 - $GC_I_SKILL_ID_SPIRITUAL_POSSESSION
Func CanUse_SpiritualPossession()
	Return True
EndFunc

Func BestTarget_SpiritualPossession($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1929 - $GC_I_SKILL_ID_WATER
Func CanUse_Water()
	Return True
EndFunc

Func BestTarget_Water($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1939 - $GC_I_SKILL_ID_SHROUD_OF_ASH
Func CanUse_ShroudOfAsh()
	Return True
EndFunc

Func BestTarget_ShroudOfAsh($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1940 - $GC_I_SKILL_ID_FLAME_CALL
Func CanUse_FlameCall()
	Return True
EndFunc

Func BestTarget_FlameCall($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1941 - $GC_I_SKILL_ID_TORTURERS_INFERNO
Func CanUse_TorturersInferno()
	Return True
EndFunc

Func BestTarget_TorturersInferno($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1942 - $GC_I_SKILL_ID_WHIRLING_FIRES
Func CanUse_WhirlingFires()
	Return True
EndFunc

Func BestTarget_WhirlingFires($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1943 - $GC_I_SKILL_ID_CHARR_SIEGE_ATTACK_WHAT_MUST_BE_DONE
Func CanUse_CharrSiegeAttackWhatMustBeDone()
	Return True
EndFunc

Func BestTarget_CharrSiegeAttackWhatMustBeDone($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1944 - $GC_I_SKILL_ID_CHARR_SIEGE_ATTACK_AGAINST_THE_CHARR
Func CanUse_CharrSiegeAttackAgainstTheCharr()
	Return True
EndFunc

Func BestTarget_CharrSiegeAttackAgainstTheCharr($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2011 - $GC_I_SKILL_ID_GRAPPLE
Func CanUse_Grapple()
	Return True
EndFunc

Func BestTarget_Grapple($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2078 - $GC_I_SKILL_ID_BERSERK
Func CanUse_Berserk()
	Return True
EndFunc

Func BestTarget_Berserk($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2080 - $GC_I_SKILL_ID_CHOMP
Func CanUse_Chomp()
	Return True
EndFunc

Func BestTarget_Chomp($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2081 - $GC_I_SKILL_ID_TWISTING_JAWS
Func CanUse_TwistingJaws()
	Return True
EndFunc

Func BestTarget_TwistingJaws($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2088 - $GC_I_SKILL_ID_TRAMPLE
Func CanUse_Trample()
	Return True
EndFunc

Func BestTarget_Trample($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2103 - $GC_I_SKILL_ID_NECROSIS
Func CanUse_Necrosis()
	Return True
EndFunc

Func BestTarget_Necrosis($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2106 - $GC_I_SKILL_ID_CALL_OF_THE_EYE
Func CanUse_CallOfTheEye()
	Return True
EndFunc

Func BestTarget_CallOfTheEye($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2113 - $GC_I_SKILL_ID_URSAN_RAGE_BLOOD_WASHES_BLOOD
Func CanUse_UrsanRageBloodWashesBlood()
	Return True
EndFunc

Func BestTarget_UrsanRageBloodWashesBlood($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2114 - $GC_I_SKILL_ID_URSAN_STRIKE_BLOOD_WASHES_BLOOD
Func CanUse_UrsanStrikeBloodWashesBlood()
	Return True
EndFunc

Func BestTarget_UrsanStrikeBloodWashesBlood($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2115 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2118 - $GC_I_SKILL_ID_FIREBOMB
Func CanUse_Firebomb()
	Return True
EndFunc

Func BestTarget_Firebomb($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2119 - $GC_I_SKILL_ID_SHIELD_OF_FIRE
Func CanUse_ShieldOfFire()
	Return True
EndFunc

Func BestTarget_ShieldOfFire($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2129 - $GC_I_SKILL_ID_VOLFEN_CLAW_CURSE_OF_THE_NORNBEAR
Func CanUse_VolfenClawCurseOfTheNornbear()
	Return True
EndFunc

Func BestTarget_VolfenClawCurseOfTheNornbear($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2130 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2133 - $GC_I_SKILL_ID_VOLFEN_BLESSING_CURSE_OF_THE_NORNBEAR
Func CanUse_VolfenBlessingCurseOfTheNornbear()
	Return True
EndFunc

Func BestTarget_VolfenBlessingCurseOfTheNornbear($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2141 - $GC_I_SKILL_ID_COMPANIONSHIP
Func CanUse_Companionship()
	Return True
EndFunc

Func BestTarget_Companionship($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2142 - $GC_I_SKILL_ID_FERAL_AGGRESSION
Func CanUse_FeralAggression()
	Return True
EndFunc

Func BestTarget_FeralAggression($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2153 - $GC_I_SKILL_ID_A_POOL_OF_WATER
Func CanUse_APoolOfWater()
	Return True
EndFunc

Func BestTarget_APoolOfWater($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2154 - $GC_I_SKILL_ID_PHASE_SHIELD_EFFECT
Func CanUse_PhaseShieldEffect()
	Return True
EndFunc

Func BestTarget_PhaseShieldEffect($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2155 - $GC_I_SKILL_ID_PHASE_SHIELD_MONSTER_SKILL
Func BestTarget_PhaseShieldMonsterSkill($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2156 - $GC_I_SKILL_ID_VITALITY_TRANSFER
Func CanUse_VitalityTransfer()
	Return True
EndFunc

Func BestTarget_VitalityTransfer($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2159 - $GC_I_SKILL_ID_ENERGY_BLAST_GOLEM
Func CanUse_EnergyBlastGolem()
	Return True
EndFunc

Func BestTarget_EnergyBlastGolem($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2160 - $GC_I_SKILL_ID_CHAOTIC_ENERGY
Func CanUse_ChaoticEnergy()
	Return True
EndFunc

Func BestTarget_ChaoticEnergy($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2168 - $GC_I_SKILL_ID_RAVEN_BLESSING_A_GATE_TOO_FAR
Func CanUse_RavenBlessingAGateTooFar()
	Return True
EndFunc

Func BestTarget_RavenBlessingAGateTooFar($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2169 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2173 - $GC_I_SKILL_ID_RAVEN_TALONS_A_GATE_TOO_FAR
Func CanUse_RavenTalonsAGateTooFar()
	Return True
EndFunc

Func BestTarget_RavenTalonsAGateTooFar($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2174 - $GC_I_SKILL_ID_ASPECT_OF_OAK
Func CanUse_AspectOfOak()
	Return True
EndFunc

Func BestTarget_AspectOfOak($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2179 - $GC_I_SKILL_ID_SUNDERING_SOULCRUSH
Func CanUse_SunderingSoulcrush()
	Return True
EndFunc

Func BestTarget_SunderingSoulcrush($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2180 - $GC_I_SKILL_ID_PYROCLASTIC_SHOT
Func CanUse_PyroclasticShot()
	Return True
EndFunc

Func BestTarget_PyroclasticShot($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2181 - $GC_I_SKILL_ID_EXPLOSIVE_FORCE
Func CanUse_ExplosiveForce()
	Return True
EndFunc

Func BestTarget_ExplosiveForce($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2185 - $GC_I_SKILL_ID_POWDER_KEG_EXPLOSION
Func CanUse_PowderKegExplosion()
	Return True
EndFunc

Func BestTarget_PowderKegExplosion($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2203 - $GC_I_SKILL_ID_SPIRITLEECH_AURA
Func CanUse_SpiritleechAura()
	Return True
EndFunc

Func BestTarget_SpiritleechAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2207 - $GC_I_SKILL_ID_INSPIRATIONAL_SPEECH
Func CanUse_InspirationalSpeech()
	Return True
EndFunc

Func BestTarget_InspirationalSpeech($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2213 - $GC_I_SKILL_ID_EAR_BITE
Func CanUse_EarBite()
	Return True
EndFunc

Func BestTarget_EarBite($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2214 - $GC_I_SKILL_ID_LOW_BLOW
Func CanUse_LowBlow()
	Return True
EndFunc

Func BestTarget_LowBlow($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2215 - $GC_I_SKILL_ID_BRAWLING_HEADBUTT
Func CanUse_BrawlingHeadbutt()
	Return True
EndFunc

Func BestTarget_BrawlingHeadbutt($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2241 - $GC_I_SKILL_ID_GELATINOUS_CORPSE_CONSUMPTION
Func CanUse_GelatinousCorpseConsumption()
	Return True
EndFunc

Func BestTarget_GelatinousCorpseConsumption($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2242 - $GC_I_SKILL_ID_GELATINOUS_MUTATION
Func CanUse_GelatinousMutation()
	Return True
EndFunc

Func BestTarget_GelatinousMutation($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2243 - $GC_I_SKILL_ID_GELATINOUS_ABSORPTION
Func CanUse_GelatinousAbsorption()
	Return True
EndFunc

Func BestTarget_GelatinousAbsorption($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2244 - $GC_I_SKILL_ID_UNSTABLE_OOZE_EXPLOSION
Func CanUse_UnstableOozeExplosion()
	Return True
EndFunc

Func BestTarget_UnstableOozeExplosion($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2246 - $GC_I_SKILL_ID_UNSTABLE_AURA
Func CanUse_UnstableAura()
	Return True
EndFunc

Func BestTarget_UnstableAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2247 - $GC_I_SKILL_ID_UNSTABLE_PULSE
Func CanUse_UnstablePulse()
	Return True
EndFunc

Func BestTarget_UnstablePulse($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2331 - $GC_I_SKILL_ID_SEARING_BREATH
Func CanUse_SearingBreath()
	Return True
EndFunc

Func BestTarget_SearingBreath($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2333 - $GC_I_SKILL_ID_BRAWLING
Func CanUse_Brawling()
	Return True
EndFunc

Func BestTarget_Brawling($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2344 - $GC_I_SKILL_ID_CALL_OF_DESTRUCTION
Func CanUse_CallOfDestruction()
	Return True
EndFunc

Func BestTarget_CallOfDestruction($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2345 - $GC_I_SKILL_ID_FLAME_JET
Func CanUse_FlameJet()
	Return True
EndFunc

Func BestTarget_FlameJet($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2346 - $GC_I_SKILL_ID_LAVA_GROUND
Func CanUse_LavaGround()
	Return True
EndFunc

Func BestTarget_LavaGround($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2347 - $GC_I_SKILL_ID_LAVA_WAVE
Func CanUse_LavaWave()
	Return True
EndFunc

Func BestTarget_LavaWave($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2349 - $GC_I_SKILL_ID_SPIRIT_SHIELD
Func CanUse_SpiritShield()
	Return True
EndFunc

Func BestTarget_SpiritShield($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2350 - $GC_I_SKILL_ID_SUMMONING_LORD
Func CanUse_SummoningLord()
	Return True
EndFunc

Func BestTarget_SummoningLord($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2351 - $GC_I_SKILL_ID_CHARM_ANIMAL_ASHLYN_SPIDERFRIEND
Func CanUse_CharmAnimalAshlynSpiderfriend()
	Return True
EndFunc

Func BestTarget_CharmAnimalAshlynSpiderfriend($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2352 - $GC_I_SKILL_ID_CHARR_SIEGE_ATTACK_ASSAULT_ON_THE_STRONGHOLD
Func CanUse_CharrSiegeAttackAssaultOnTheStronghold()
	Return True
EndFunc

Func BestTarget_CharrSiegeAttackAssaultOnTheStronghold($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2362 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2363 - $GC_I_SKILL_ID_TALON_STRIKE
Func CanUse_TalonStrike()
	Return True
EndFunc

Func BestTarget_TalonStrike($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2364 - $GC_I_SKILL_ID_LAVA_BLAST
Func CanUse_LavaBlast()
	Return True
EndFunc

Func BestTarget_LavaBlast($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2367 - $GC_I_SKILL_ID_ALKARS_CONCOCTION
Func CanUse_AlkarsConcoction()
	Return True
EndFunc

Func BestTarget_AlkarsConcoction($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2375 - $GC_I_SKILL_ID_URSAN_STRIKE
Func CanUse_UrsanStrike()
	Return True
EndFunc

Func BestTarget_UrsanStrike($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2376 - $GC_I_SKILL_ID_URSAN_RAGE
Func CanUse_UrsanRage()
	Return True
EndFunc

Func BestTarget_UrsanRage($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2380 - $GC_I_SKILL_ID_VOLFEN_CLAW
Func CanUse_VolfenClaw()
	Return True
EndFunc

Func BestTarget_VolfenClaw($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2385 - $GC_I_SKILL_ID_RAVEN_TALONS
Func CanUse_RavenTalons()
	Return True
EndFunc

Func BestTarget_RavenTalons($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2389 - $GC_I_SKILL_ID_TOTEM_OF_MAN
Func CanUse_TotemOfMan()
	Return True
EndFunc

Func BestTarget_TotemOfMan($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2392 - $GC_I_SKILL_ID_SPAWN_PODS
Func CanUse_SpawnPods()
	Return True
EndFunc

Func BestTarget_SpawnPods($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2393 - $GC_I_SKILL_ID_ENRAGED_BLAST
Func CanUse_EnragedBlast()
	Return True
EndFunc

Func BestTarget_EnragedBlast($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2394 - $GC_I_SKILL_ID_SPAWN_HATCHLING
Func CanUse_SpawnHatchling()
	Return True
EndFunc

Func BestTarget_SpawnHatchling($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2397 - $GC_I_SKILL_ID_URSAN_AURA
Func CanUse_UrsanAura()
	Return True
EndFunc

Func BestTarget_UrsanAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2409 - $GC_I_SKILL_ID_EXTRACT_INSCRIPTION
Func CanUse_ExtractInscription()
	Return True
EndFunc

Func BestTarget_ExtractInscription($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2416 - $GC_I_SKILL_ID_AIR_OF_SUPERIORITY
Func CanUse_AirOfSuperiority()
	Return True
EndFunc

Func BestTarget_AirOfSuperiority($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2426 - $GC_I_SKILL_ID_DECIPHER_INSCRIPTIONS
Func CanUse_DecipherInscriptions()
	Return True
EndFunc

Func BestTarget_DecipherInscriptions($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2428 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2429 - $GC_I_SKILL_ID_ASURAN_FLAME_STAFF
Func CanUse_AsuranFlameStaff()
	Return True
EndFunc

Func BestTarget_AsuranFlameStaff($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2433 - $GC_I_SKILL_ID_HAUNTED_GROUND
Func CanUse_HauntedGround()
	Return True
EndFunc

Func BestTarget_HauntedGround($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2483 - $GC_I_SKILL_ID_GLOAT
Func CanUse_Gloat()
	Return True
EndFunc

Func BestTarget_Gloat($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2484 - $GC_I_SKILL_ID_METAMORPHOSIS
Func CanUse_Metamorphosis()
	Return True
EndFunc

Func BestTarget_Metamorphosis($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2485 - $GC_I_SKILL_ID_INNER_FIRE
Func CanUse_InnerFire()
	Return True
EndFunc

Func BestTarget_InnerFire($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2488 - $GC_I_SKILL_ID_FUNGAL_EXPLOSION
Func CanUse_FungalExplosion()
	Return True
EndFunc

Func BestTarget_FungalExplosion($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2489 - $GC_I_SKILL_ID_BLOOD_RAGE
Func CanUse_BloodRage()
	Return True
EndFunc

Func BestTarget_BloodRage($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2492 - $GC_I_SKILL_ID_OOZE_COMBINATION
Func CanUse_OozeCombination()
	Return True
EndFunc

Func BestTarget_OozeCombination($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2493 - $GC_I_SKILL_ID_OOZE_DIVISION
Func CanUse_OozeDivision()
	Return True
EndFunc

Func BestTarget_OozeDivision($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2496 - $GC_I_SKILL_ID_SPORE_EXPLOSION
Func CanUse_SporeExplosion()
	Return True
EndFunc

Func BestTarget_SporeExplosion($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2497 - $GC_I_SKILL_ID_DORMANT_HUSK
Func CanUse_DormantHusk()
	Return True
EndFunc

Func BestTarget_DormantHusk($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2498 - $GC_I_SKILL_ID_MONKEY_SEE__MONKEY_DO
Func CanUse_MonkeySeeMonkeyDo()
	Return True
EndFunc

Func BestTarget_MonkeySeeMonkeyDo($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2499 - $GC_I_SKILL_ID_FEEDING_FRENZY
Func CanUse_FeedingFrenzy()
	Return True
EndFunc

Func BestTarget_FeedingFrenzy($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2502 - $GC_I_SKILL_ID_SOULRENDING_SHRIEK
Func CanUse_SoulrendingShriek()
	Return True
EndFunc

Func BestTarget_SoulrendingShriek($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2505 - $GC_I_SKILL_ID_SIEGE_DEVOURER_FEAST
Func CanUse_SiegeDevourerFeast()
	Return True
EndFunc

Func BestTarget_SiegeDevourerFeast($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2506 - $GC_I_SKILL_ID_DEVOURER_BITE
Func CanUse_DevourerBite()
	Return True
EndFunc

Func BestTarget_DevourerBite($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2507 - $GC_I_SKILL_ID_SIEGE_DEVOURER_SWIPE
Func CanUse_SiegeDevourerSwipe()
	Return True
EndFunc

Func BestTarget_SiegeDevourerSwipe($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2508 - $GC_I_SKILL_ID_DEVOURER_SIEGE
Func CanUse_DevourerSiege()
	Return True
EndFunc

Func BestTarget_DevourerSiege($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2513 - $GC_I_SKILL_ID_DISMOUNT_SIEGE_DEVOURER
Func CanUse_DismountSiegeDevourer()
	Return True
EndFunc

Func BestTarget_DismountSiegeDevourer($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2516 - $GC_I_SKILL_ID_MOUNT
Func CanUse_Mount()
	Return True
EndFunc

Func BestTarget_Mount($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2518 - $GC_I_SKILL_ID_TENGUS_GAZE
Func CanUse_TengusGaze()
	Return True
EndFunc

Func BestTarget_TengusGaze($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2524 - $GC_I_SKILL_ID_FORGEWIGHTS_BLESSING
Func CanUse_ForgewightsBlessing()
	Return True
EndFunc

Func BestTarget_ForgewightsBlessing($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2525 - $GC_I_SKILL_ID_SELVETARMS_BLESSING
Func CanUse_SelvetarmsBlessing()
	Return True
EndFunc

Func BestTarget_SelvetarmsBlessing($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2526 - $GC_I_SKILL_ID_THOMMISS_BLESSING
Func CanUse_ThommissBlessing()
	Return True
EndFunc

Func BestTarget_ThommissBlessing($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2529 - $GC_I_SKILL_ID_RANDS_ATTACK
Func CanUse_RandsAttack()
	Return True
EndFunc

Func BestTarget_RandsAttack($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2542 - $GC_I_SKILL_ID_CHARM_ANIMAL_MONSTER_
; Skill ID: 2545 - $GC_I_SKILL_ID_LIT_TORCH
Func CanUse_LitTorch()
	Return True
EndFunc

Func BestTarget_LitTorch($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2650 - $GC_I_SKILL_ID_CHARM_ANIMAL_CHARR_DEMOLISHER
Func CanUse_CharmAnimalCharrDemolisher()
	Return True
EndFunc

Func BestTarget_CharmAnimalCharrDemolisher($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2661 - $GC_I_SKILL_ID_THROW_ROCK
Func CanUse_ThrowRock()
	Return True
EndFunc

Func BestTarget_ThrowRock($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2662 - $GC_I_SKILL_ID_NIGHTMARISH_AURA
Func CanUse_NightmarishAura()
	Return True
EndFunc

Func BestTarget_NightmarishAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2663 - $GC_I_SKILL_ID_SIEGE_STRIKE
Func CanUse_SiegeStrike()
	Return True
EndFunc

Func BestTarget_SiegeStrike($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2665 - $GC_I_SKILL_ID_BARBED_BOMB
Func CanUse_BarbedBomb()
	Return True
EndFunc

Func BestTarget_BarbedBomb($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2667 - $GC_I_SKILL_ID_BALM_BOMB
Func CanUse_BalmBomb()
	Return True
EndFunc

Func BestTarget_BalmBomb($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2668 - $GC_I_SKILL_ID_EXPLOSIVES
Func CanUse_Explosives()
	Return True
EndFunc

Func BestTarget_Explosives($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2669 - $GC_I_SKILL_ID_RATIONS
Func CanUse_Rations()
	Return True
EndFunc

Func BestTarget_Rations($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2673 - $GC_I_SKILL_ID_STUN_BOMB
Func CanUse_StunBomb()
	Return True
EndFunc

Func BestTarget_StunBomb($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2677 - $GC_I_SKILL_ID_GIANT_STOMP_TURAI_OSSA
Func CanUse_GiantStompTuraiOssa()
	Return True
EndFunc

Func BestTarget_GiantStompTuraiOssa($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2679 - $GC_I_SKILL_ID_JUNUNDU_SIEGE2
Func CanUse_JununduSiege2()
	Return True
EndFunc

Func BestTarget_JununduSiege2($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2692 - $GC_I_SKILL_ID_FIRE_DART2
Func CanUse_FireDart2()
	Return True
EndFunc

Func BestTarget_FireDart2($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2697 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2831 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2867 - $GC_I_SKILL_ID_ANCESTORS_RAGE_PVP
Func CanUse_AncestorsRagePvp()
	Return True
EndFunc

Func BestTarget_AncestorsRagePvp($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2894 - $GC_I_SKILL_ID_BAMPH_LITE
Func BestTarget_BamphLite($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2903 - $GC_I_SKILL_ID_REACTOR_BLAST_TIMER
Func CanUse_ReactorBlastTimer()
	Return True
EndFunc

Func BestTarget_ReactorBlastTimer($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2905 - $GC_I_SKILL_ID_INTERNAL_POWER_ENGAGED
Func CanUse_InternalPowerEngaged()
	Return True
EndFunc

Func BestTarget_InternalPowerEngaged($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2910 - $GC_I_SKILL_ID_COUNTDOWN
Func CanUse_Countdown()
	Return True
EndFunc

Func BestTarget_Countdown($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2918 - $GC_I_SKILL_ID_NOX_LOCK_ON
Func CanUse_NoxLockOn()
	Return True
EndFunc

Func BestTarget_NoxLockOn($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2926 - $GC_I_SKILL_ID_BAMPH_LIFESTEAL
Func BestTarget_BamphLifesteal($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2931 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2960 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2961 - $GC_I_SKILL_ID_DELAYED_BLAST_BAMPH
Func CanUse_DelayedBlastBamph()
	Return True
EndFunc

Func BestTarget_DelayedBlastBamph($aAggroRange)
	Return 0
EndFunc

; Skill ID: 3000 - $GC_I_SKILL_ID_GUNTHERS_GAZE
Func CanUse_GunthersGaze()
	Return True
EndFunc

Func BestTarget_GunthersGaze($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3045 - $GC_I_SKILL_ID_COMFORT_ANIMAL_PVP
Func CanUse_ComfortAnimalPvp()
	Return True
EndFunc

Func BestTarget_ComfortAnimalPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3065 - $GC_I_SKILL_ID_CHARM_ANIMAL2
Func CanUse_CharmAnimal2()
	Return True
EndFunc

Func BestTarget_CharmAnimal2($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3066 - $GC_I_SKILL_ID_CHARM_ANIMAL3
Func CanUse_CharmAnimal3()
	Return True
EndFunc

Func BestTarget_CharmAnimal3($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3068 - $GC_I_SKILL_ID_CHARM_ANIMAL_CODEX
Func CanUse_CharmAnimalCodex()
	Return True
EndFunc

Func BestTarget_CharmAnimalCodex($aAggroRange)
	Return 0
EndFunc

; Skill ID: 3083 - $GC_I_SKILL_ID_TOUCH_OF_DHUUM
Func CanUse_TouchOfDhuum()
	Return True
EndFunc

Func BestTarget_TouchOfDhuum($aAggroRange)
	Return 0
EndFunc

; Skill ID: 3144 - $GC_I_SKILL_ID_HEAL_AS_ONE_PVP
Func CanUse_HealAsOnePvp()
	Return True
EndFunc

Func BestTarget_HealAsOnePvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3159 - $GC_I_SKILL_ID_CHARM_DRAKE
Func CanUse_CharmDrake()
	Return True
EndFunc

Func BestTarget_CharmDrake($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3160 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 3161 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 3175 - $GC_I_SKILL_ID_GUILD_MONUMENT_PROTECTED
Func CanUse_GuildMonumentProtected()
	Return True
EndFunc

Func BestTarget_GuildMonumentProtected($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3199 - $GC_I_SKILL_ID_SACRIFICE_PAWN
Func CanUse_SacrificePawn()
	Return True
EndFunc

Func BestTarget_SacrificePawn($aAggroRange)
	Return 0
EndFunc

; Skill ID: 3203 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 3205 - $GC_I_SKILL_ID_ENTOURAGE
Func CanUse_Entourage()
	Return True
EndFunc

Func BestTarget_Entourage($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3207 - $GC_I_SKILL_ID_ENTOURAGE_BUFFER
Func CanUse_EntourageBuffer()
	Return True
EndFunc

Func BestTarget_EntourageBuffer($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3209 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 3303 - $GC_I_SKILL_ID_DAMAGE_ASSESSMENT
Func CanUse_DamageAssessment()
	Return True
EndFunc

Func BestTarget_DamageAssessment($aAggroRange)
	Return 0
EndFunc

; Skill ID: 3305 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 3370 - $GC_I_SKILL_ID_SMASH_OF_THE_TITANS
Func CanUse_SmashOfTheTitans()
	Return True
EndFunc

Func BestTarget_SmashOfTheTitans($aAggroRange)
	Return 0
EndFunc

; Skill ID: 3384 - $GC_I_SKILL_ID_ANNIHILATOR_TOSS
Func CanUse_AnnihilatorToss()
	Return True
EndFunc

Func BestTarget_AnnihilatorToss($aAggroRange)
	Return 0
EndFunc

; Skill ID: 3402 - $GC_I_SKILL_ID_TONIC_TIPSINESS
Func CanUse_TonicTipsiness()
	Return True
EndFunc

Func BestTarget_TonicTipsiness($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3404 - $GC_I_SKILL_ID_GIFT_OF_BATTLE
Func CanUse_GiftOfBattle()
	Return True
EndFunc

Func BestTarget_GiftOfBattle($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3410 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 3417 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 3418 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 3419 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 3428 - $GC_I_SKILL_ID_SHADOW_THEFT
Func CanUse_ShadowTheft()
	Return True
EndFunc

Func BestTarget_ShadowTheft($aAggroRange)
	Return 0
EndFunc

