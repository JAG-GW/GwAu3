#include-once

; Skill ID: 32 - $GC_I_SKILL_ID_ILLUSION_OF_WEAKNESS
Func CanUse_IllusionOfWeakness()
	Return True
EndFunc

Func BestTarget_IllusionOfWeakness($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 33 - $GC_I_SKILL_ID_ILLUSIONARY_WEAPONRY
Func CanUse_IllusionaryWeaponry()
	Return True
EndFunc

Func BestTarget_IllusionaryWeaponry($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 34 - $GC_I_SKILL_ID_SYMPATHETIC_VISAGE
Func CanUse_SympatheticVisage()
	Return True
EndFunc

Func BestTarget_SympatheticVisage($aAggroRange)
	Return 0
EndFunc

; Skill ID: 37 - $GC_I_SKILL_ID_ILLUSION_OF_HASTE
Func CanUse_IllusionOfHaste()
	Return True
EndFunc

Func BestTarget_IllusionOfHaste($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 38 - $GC_I_SKILL_ID_CHANNELING
Func CanUse_Channeling()
	Return True
EndFunc

Func BestTarget_Channeling($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 74 - $GC_I_SKILL_ID_ECHO
Func CanUse_Echo()
	Return True
EndFunc

Func BestTarget_Echo($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 75 - $GC_I_SKILL_ID_ARCANE_ECHO
Func CanUse_ArcaneEcho()
	Return True
EndFunc

Func BestTarget_ArcaneEcho($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 82 - $GC_I_SKILL_ID_MANTRA_OF_RECALL
Func CanUse_MantraOfRecall()
	Return True
EndFunc

Func BestTarget_MantraOfRecall($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 88 - $GC_I_SKILL_ID_VERATAS_AURA
Func CanUse_VeratasAura()
	Return True
EndFunc

Func BestTarget_VeratasAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 104 - $GC_I_SKILL_ID_DEATH_NOVA
Func CanUse_DeathNova()
	Return True
EndFunc

Func BestTarget_DeathNova($aAggroRange)
	Return 0
EndFunc

; Skill ID: 111 - $GC_I_SKILL_ID_AWAKEN_THE_BLOOD
Func CanUse_AwakenTheBlood()
	Return True
EndFunc

Func BestTarget_AwakenTheBlood($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 113 - $GC_I_SKILL_ID_TAINTED_FLESH
Func CanUse_TaintedFlesh()
	Return True
EndFunc

Func BestTarget_TaintedFlesh($aAggroRange)
	Return 0
EndFunc

; Skill ID: 114 - $GC_I_SKILL_ID_AURA_OF_THE_LICH
Func CanUse_AuraOfTheLich()
	Return True
EndFunc

Func BestTarget_AuraOfTheLich($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 115 - $GC_I_SKILL_ID_BLOOD_RENEWAL
Func CanUse_BloodRenewal()
	Return True
EndFunc

Func BestTarget_BloodRenewal($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 116 - $GC_I_SKILL_ID_DARK_AURA
Func CanUse_DarkAura()
	Return True
EndFunc

Func BestTarget_DarkAura($aAggroRange)
	Return 0
EndFunc

; Skill ID: 119 - $GC_I_SKILL_ID_BLOOD_IS_POWER
Func CanUse_BloodIsPower()
	Return True
EndFunc

Func BestTarget_BloodIsPower($aAggroRange)
	Return 0
EndFunc

; Skill ID: 130 - $GC_I_SKILL_ID_DEMONIC_FLESH
Func CanUse_DemonicFlesh()
	Return True
EndFunc

Func BestTarget_DemonicFlesh($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 134 - $GC_I_SKILL_ID_ORDER_OF_PAIN
Func CanUse_OrderOfPain()
	Return True
EndFunc

Func BestTarget_OrderOfPain($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 138 - $GC_I_SKILL_ID_DARK_BOND
Func CanUse_DarkBond()
	Return True
EndFunc

Func BestTarget_DarkBond($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 139 - $GC_I_SKILL_ID_INFUSE_CONDITION
Func CanUse_InfuseCondition()
	Return True
EndFunc

Func BestTarget_InfuseCondition($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 147 - $GC_I_SKILL_ID_DARK_FURY
Func CanUse_DarkFury()
	Return True
EndFunc

Func BestTarget_DarkFury($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 148 - $GC_I_SKILL_ID_ORDER_OF_THE_VAMPIRE
Func CanUse_OrderOfTheVampire()
	Return True
EndFunc

Func BestTarget_OrderOfTheVampire($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 157 - $GC_I_SKILL_ID_BLOOD_RITUAL
Func CanUse_BloodRitual()
	Return True
EndFunc

Func BestTarget_BloodRitual($aAggroRange)
	Return 0
EndFunc

; Skill ID: 160 - $GC_I_SKILL_ID_WINDBORNE_SPEED
Func CanUse_WindborneSpeed()
	Return True
EndFunc

Func BestTarget_WindborneSpeed($aAggroRange)
	Return 0
EndFunc

; Skill ID: 164 - $GC_I_SKILL_ID_ELEMENTAL_ATTUNEMENT
Func CanUse_ElementalAttunement()
	Return True
EndFunc

Func BestTarget_ElementalAttunement($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 165 - $GC_I_SKILL_ID_ARMOR_OF_EARTH
Func CanUse_ArmorOfEarth()
	Return True
EndFunc

Func BestTarget_ArmorOfEarth($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 166 - $GC_I_SKILL_ID_KINETIC_ARMOR
Func CanUse_KineticArmor()
	Return True
EndFunc

Func BestTarget_KineticArmor($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 168 - $GC_I_SKILL_ID_MAGNETIC_AURA
Func CanUse_MagneticAura()
	Return True
EndFunc

Func BestTarget_MagneticAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 169 - $GC_I_SKILL_ID_EARTH_ATTUNEMENT
Func CanUse_EarthAttunement()
	Return True
EndFunc

Func BestTarget_EarthAttunement($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 178 - $GC_I_SKILL_ID_ETHER_PRODIGY
Func CanUse_EtherProdigy()
	Return True
EndFunc

Func BestTarget_EtherProdigy($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 180 - $GC_I_SKILL_ID_AURA_OF_RESTORATION
Func CanUse_AuraOfRestoration()
	Return True
EndFunc

Func BestTarget_AuraOfRestoration($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 181 - $GC_I_SKILL_ID_ETHER_RENEWAL
Func CanUse_EtherRenewal()
	Return True
EndFunc

Func BestTarget_EtherRenewal($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 182 - $GC_I_SKILL_ID_CONJURE_FLAME
Func CanUse_ConjureFlame()
	Return True
EndFunc

Func BestTarget_ConjureFlame($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 184 - $GC_I_SKILL_ID_FIRE_ATTUNEMENT
Func CanUse_FireAttunement()
	Return True
EndFunc

Func BestTarget_FireAttunement($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 206 - $GC_I_SKILL_ID_ARMOR_OF_FROST
Func CanUse_ArmorOfFrost()
	Return True
EndFunc

Func BestTarget_ArmorOfFrost($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 207 - $GC_I_SKILL_ID_CONJURE_FROST
Func CanUse_ConjureFrost()
	Return True
EndFunc

Func BestTarget_ConjureFrost($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 208 - $GC_I_SKILL_ID_WATER_ATTUNEMENT
Func CanUse_WaterAttunement()
	Return True
EndFunc

Func BestTarget_WaterAttunement($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 214 - $GC_I_SKILL_ID_ICE_SPEAR
Func CanUse_IceSpear()
	Return True
EndFunc

Func BestTarget_IceSpear($aAggroRange)
	Return 0
EndFunc

; Skill ID: 216 - $GC_I_SKILL_ID_IRON_MIST
Func CanUse_IronMist()
	Return True
EndFunc

Func BestTarget_IronMist($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 218 - $GC_I_SKILL_ID_OBSIDIAN_FLESH
Func CanUse_ObsidianFlesh()
	Return True
EndFunc

Func BestTarget_ObsidianFlesh($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 221 - $GC_I_SKILL_ID_CONJURE_LIGHTNING
Func CanUse_ConjureLightning()
	Return True
EndFunc

Func BestTarget_ConjureLightning($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 225 - $GC_I_SKILL_ID_AIR_ATTUNEMENT
Func CanUse_AirAttunement()
	Return True
EndFunc

Func BestTarget_AirAttunement($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 233 - $GC_I_SKILL_ID_SWIRLING_AURA
Func CanUse_SwirlingAura()
	Return True
EndFunc

Func BestTarget_SwirlingAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 236 - $GC_I_SKILL_ID_MIST_FORM
Func CanUse_MistForm()
	Return True
EndFunc

Func BestTarget_MistForm($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 238 - $GC_I_SKILL_ID_ARMOR_OF_MIST
Func CanUse_ArmorOfMist()
	Return True
EndFunc

Func BestTarget_ArmorOfMist($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 241 - $GC_I_SKILL_ID_LIFE_BOND
Func CanUse_LifeBond()
	Return True
EndFunc

Func BestTarget_LifeBond($aAggroRange)
	Return 0
EndFunc

; Skill ID: 242 - $GC_I_SKILL_ID_BALTHAZARS_SPIRIT
Func CanUse_BalthazarsSpirit()
	Return True
EndFunc

Func BestTarget_BalthazarsSpirit($aAggroRange)
	Return 0
EndFunc

; Skill ID: 243 - $GC_I_SKILL_ID_STRENGTH_OF_HONOR
Func CanUse_StrengthOfHonor()
	Return True
EndFunc

Func BestTarget_StrengthOfHonor($aAggroRange)
	Return 0
EndFunc

; Skill ID: 244 - $GC_I_SKILL_ID_LIFE_ATTUNEMENT
Func CanUse_LifeAttunement()
	Return True
EndFunc

Func BestTarget_LifeAttunement($aAggroRange)
	Return 0
EndFunc

; Skill ID: 245 - $GC_I_SKILL_ID_PROTECTIVE_SPIRIT
Func CanUse_ProtectiveSpirit()
	Return True
EndFunc

Func BestTarget_ProtectiveSpirit($aAggroRange)
	Return 0
EndFunc

; Skill ID: 246 - $GC_I_SKILL_ID_DIVINE_INTERVENTION
Func CanUse_DivineIntervention()
	Return True
EndFunc

Func BestTarget_DivineIntervention($aAggroRange)
	Return 0
EndFunc

; Skill ID: 248 - $GC_I_SKILL_ID_RETRIBUTION
Func CanUse_Retribution()
	Return True
EndFunc

Func BestTarget_Retribution($aAggroRange)
	Return 0
EndFunc

; Skill ID: 249 - $GC_I_SKILL_ID_HOLY_WRATH
Func CanUse_HolyWrath()
	Return True
EndFunc

Func BestTarget_HolyWrath($aAggroRange)
	Return 0
EndFunc

; Skill ID: 250 - $GC_I_SKILL_ID_ESSENCE_BOND
Func CanUse_EssenceBond()
	Return True
EndFunc

Func BestTarget_EssenceBond($aAggroRange)
	Return 0
EndFunc

; Skill ID: 254 - $GC_I_SKILL_ID_VIGOROUS_SPIRIT
Func CanUse_VigorousSpirit()
	Return True
EndFunc

Func BestTarget_VigorousSpirit($aAggroRange)
	Return 0
EndFunc

; Skill ID: 255 - $GC_I_SKILL_ID_WATCHFUL_SPIRIT
Func CanUse_WatchfulSpirit()
	Return True
EndFunc

Func BestTarget_WatchfulSpirit($aAggroRange)
	Return 0
EndFunc

; Skill ID: 256 - $GC_I_SKILL_ID_BLESSED_AURA
Func CanUse_BlessedAura()
	Return True
EndFunc

Func BestTarget_BlessedAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 257 - $GC_I_SKILL_ID_AEGIS
Func CanUse_Aegis()
	Return True
EndFunc

Func BestTarget_Aegis($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 258 - $GC_I_SKILL_ID_GUARDIAN
Func CanUse_Guardian()
	Return True
EndFunc

Func BestTarget_Guardian($aAggroRange)
	Return 0
EndFunc

; Skill ID: 259 - $GC_I_SKILL_ID_SHIELD_OF_DEFLECTION
Func CanUse_ShieldOfDeflection()
	Return True
EndFunc

Func BestTarget_ShieldOfDeflection($aAggroRange)
	Return 0
EndFunc

; Skill ID: 260 - $GC_I_SKILL_ID_AURA_OF_FAITH
Func CanUse_AuraOfFaith()
	Return True
EndFunc

Func BestTarget_AuraOfFaith($aAggroRange)
	Return 0
EndFunc

; Skill ID: 261 - $GC_I_SKILL_ID_SHIELD_OF_REGENERATION
Func CanUse_ShieldOfRegeneration()
	Return True
EndFunc

Func BestTarget_ShieldOfRegeneration($aAggroRange)
	Return 0
EndFunc

; Skill ID: 262 - $GC_I_SKILL_ID_SHIELD_OF_JUDGMENT
Func CanUse_ShieldOfJudgment()
	Return True
EndFunc

Func BestTarget_ShieldOfJudgment($aAggroRange)
	Return 0
EndFunc

; Skill ID: 263 - $GC_I_SKILL_ID_PROTECTIVE_BOND
Func CanUse_ProtectiveBond()
	Return True
EndFunc

Func BestTarget_ProtectiveBond($aAggroRange)
	Return 0
EndFunc

; Skill ID: 266 - $GC_I_SKILL_ID_PEACE_AND_HARMONY
Func CanUse_PeaceAndHarmony()
	Return True
EndFunc

Func BestTarget_PeaceAndHarmony($aAggroRange)
	Return 0
EndFunc

; Skill ID: 267 - $GC_I_SKILL_ID_JUDGES_INSIGHT
Func CanUse_JudgesInsight()
	Return True
EndFunc

Func BestTarget_JudgesInsight($aAggroRange)
	Return 0
EndFunc

; Skill ID: 268 - $GC_I_SKILL_ID_UNYIELDING_AURA
Func CanUse_UnyieldingAura()
	Return True
EndFunc

Func BestTarget_UnyieldingAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 269 - $GC_I_SKILL_ID_MARK_OF_PROTECTION
Func CanUse_MarkOfProtection()
	Return True
EndFunc

Func BestTarget_MarkOfProtection($aAggroRange)
	Return 0
EndFunc

; Skill ID: 270 - $GC_I_SKILL_ID_LIFE_BARRIER
Func CanUse_LifeBarrier()
	Return True
EndFunc

Func BestTarget_LifeBarrier($aAggroRange)
	Return 0
EndFunc

; Skill ID: 271 - $GC_I_SKILL_ID_ZEALOTS_FIRE
Func CanUse_ZealotsFire()
	Return True
EndFunc

Func BestTarget_ZealotsFire($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 272 - $GC_I_SKILL_ID_BALTHAZARS_AURA
Func CanUse_BalthazarsAura()
	Return True
EndFunc

Func BestTarget_BalthazarsAura($aAggroRange)
	Return 0
EndFunc

; Skill ID: 273 - $GC_I_SKILL_ID_SPELL_BREAKER
Func CanUse_SpellBreaker()
	Return True
EndFunc

Func BestTarget_SpellBreaker($aAggroRange)
	Return 0
EndFunc

; Skill ID: 274 - $GC_I_SKILL_ID_HEALING_SEED
Func CanUse_HealingSeed()
	Return True
EndFunc

Func BestTarget_HealingSeed($aAggroRange)
	Return 0
EndFunc

; Skill ID: 284 - $GC_I_SKILL_ID_DIVINE_BOON
Func CanUse_DivineBoon()
	Return True
EndFunc

Func BestTarget_DivineBoon($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 285 - $GC_I_SKILL_ID_HEALING_HANDS
Func CanUse_HealingHands()
	Return True
EndFunc

Func BestTarget_HealingHands($aAggroRange)
	Return 0
EndFunc

; Skill ID: 288 - $GC_I_SKILL_ID_HEALING_BREEZE
Func CanUse_HealingBreeze()
	Return True
EndFunc

Func BestTarget_HealingBreeze($aAggroRange)
	Return 0
EndFunc

; Skill ID: 289 - $GC_I_SKILL_ID_VITAL_BLESSING
Func CanUse_VitalBlessing()
	Return True
EndFunc

Func BestTarget_VitalBlessing($aAggroRange)
	Return 0
EndFunc

; Skill ID: 290 - $GC_I_SKILL_ID_MENDING
Func CanUse_Mending()
	Return True
EndFunc

Func BestTarget_Mending($aAggroRange)
	Return 0
EndFunc

; Skill ID: 291 - $GC_I_SKILL_ID_LIVE_VICARIOUSLY
Func CanUse_LiveVicariously()
	Return True
EndFunc

Func BestTarget_LiveVicariously($aAggroRange)
	Return 0
EndFunc

; Skill ID: 299 - $GC_I_SKILL_ID_SHIELDING_HANDS
Func CanUse_ShieldingHands()
	Return True
EndFunc

Func BestTarget_ShieldingHands($aAggroRange)
	Return 0
EndFunc

; Skill ID: 307 - $GC_I_SKILL_ID_REVERSAL_OF_FORTUNE
Func CanUse_ReversalOfFortune()
	Return True
EndFunc

Func BestTarget_ReversalOfFortune($aAggroRange)
	Return 0
EndFunc

; Skill ID: 308 - $GC_I_SKILL_ID_SUCCOR
Func CanUse_Succor()
	Return True
EndFunc

Func BestTarget_Succor($aAggroRange)
	Return 0
EndFunc

; Skill ID: 309 - $GC_I_SKILL_ID_HOLY_VEIL
Func CanUse_HolyVeil()
	Return True
EndFunc

Func BestTarget_HolyVeil($aAggroRange)
	Return 0
EndFunc

; Skill ID: 310 - $GC_I_SKILL_ID_DIVINE_SPIRIT
Func CanUse_DivineSpirit()
	Return True
EndFunc

Func BestTarget_DivineSpirit($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 315 - $GC_I_SKILL_ID_VENGEANCE
Func CanUse_Vengeance()
	If CachedAgent_HasEffect($GC_I_SKILL_ID_CURSE_OF_DHUUM) Or CachedAgent_HasEffect($GC_I_SKILL_ID_FROZEN_SOIL) Then Return False
	Return True
EndFunc

Func BestTarget_Vengeance($aAggroRange)
	Return 0
EndFunc

; Skill ID: 490 - $GC_I_SKILL_ID_VITAL_BLESSING_MONSTER_SKILL
Func BestTarget_VitalBlessingMonsterSkill($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 515 - $GC_I_SKILL_ID_CHARR_BUFF
Func CanUse_CharrBuff()
	Return True
EndFunc

Func BestTarget_CharrBuff($aAggroRange)
	Return 0
EndFunc

; Skill ID: 532 - $GC_I_SKILL_ID_HEALING_BREEZE_AGNARS_RAGE
Func CanUse_HealingBreezeAgnarsRage()
	Return True
EndFunc

Func BestTarget_HealingBreezeAgnarsRage($aAggroRange)
	Return 0
EndFunc

; Skill ID: 584 - $GC_I_SKILL_ID_DIVINE_FIRE
Func CanUse_DivineFire()
	Return True
EndFunc

Func BestTarget_DivineFire($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 596 - $GC_I_SKILL_ID_CHIMERA_OF_INTENSITY
Func CanUse_ChimeraOfIntensity()
	Return True
EndFunc

Func BestTarget_ChimeraOfIntensity($aAggroRange)
	Return 0
EndFunc

; Skill ID: 763 - $GC_I_SKILL_ID_JAUNDICED_GAZE
Func CanUse_JaundicedGaze()
	Return True
EndFunc

Func BestTarget_JaundicedGaze($aAggroRange)
	Return 0
EndFunc

; Skill ID: 771 - $GC_I_SKILL_ID_AURA_OF_DISPLACEMENT
Func CanUse_AuraOfDisplacement()
	Return True
EndFunc

Func BestTarget_AuraOfDisplacement($aAggroRange)
	Return 0
EndFunc

; Skill ID: 806 - $GC_I_SKILL_ID_CULTISTS_FERVOR
Func CanUse_CultistsFervor()
	Return True
EndFunc

Func BestTarget_CultistsFervor($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 807 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 813 - $GC_I_SKILL_ID_LYSSAS_AURA
Func CanUse_LyssasAura()
	Return True
EndFunc

Func BestTarget_LyssasAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 814 - $GC_I_SKILL_ID_SHADOW_REFUGE
Func CanUse_ShadowRefuge()
	Return True
EndFunc

Func BestTarget_ShadowRefuge($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 819 - $GC_I_SKILL_ID_VAMPIRIC_SPIRIT
Func CanUse_VampiricSpirit()
	Return True
EndFunc

Func BestTarget_VampiricSpirit($aAggroRange)
	Return 0
EndFunc

; Skill ID: 823 - $GC_I_SKILL_ID_BURNING_SPEED
Func CanUse_BurningSpeed()
	Return True
EndFunc

Func BestTarget_BurningSpeed($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 826 - $GC_I_SKILL_ID_SHADOW_FORM
Func CanUse_ShadowForm()
	Return True
EndFunc

Func BestTarget_ShadowForm($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 829 - $GC_I_SKILL_ID_VERATAS_PROMISE
Func CanUse_VeratasPromise()
	Return True
EndFunc

Func BestTarget_VeratasPromise($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 833 - $GC_I_SKILL_ID_BORROWED_ENERGY
Func CanUse_BorrowedEnergy()
	Return True
EndFunc

Func BestTarget_BorrowedEnergy($aAggroRange)
	Return 0
EndFunc

; Skill ID: 837 - $GC_I_SKILL_ID_ENERGY_BOON
Func CanUse_EnergyBoon()
	Return True
EndFunc

Func BestTarget_EnergyBoon($aAggroRange)
	Return 0
EndFunc

; Skill ID: 838 - $GC_I_SKILL_ID_DWAYNAS_SORROW
Func CanUse_DwaynasSorrow()
	Return True
EndFunc

Func BestTarget_DwaynasSorrow($aAggroRange)
	Return 0
EndFunc

; Skill ID: 843 - $GC_I_SKILL_ID_GUST
Func CanUse_Gust()
	Return True
EndFunc

Func BestTarget_Gust($aAggroRange)
	Return 0
EndFunc

; Skill ID: 848 - $GC_I_SKILL_ID_REVERSE_HEX
Func CanUse_ReverseHex()
	Return True
EndFunc

Func BestTarget_ReverseHex($aAggroRange)
	Return 0
EndFunc

; Skill ID: 863 - $GC_I_SKILL_ID_ORDER_OF_APOSTASY
Func CanUse_OrderOfApostasy()
	Return True
EndFunc

Func BestTarget_OrderOfApostasy($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 885 - $GC_I_SKILL_ID_SHIELD_GUARDIAN
Func CanUse_ShieldGuardian()
	Return True
EndFunc

Func BestTarget_ShieldGuardian($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 886 - $GC_I_SKILL_ID_RESTFUL_BREEZE
Func CanUse_RestfulBreeze()
	Return True
EndFunc

Func BestTarget_RestfulBreeze($aAggroRange)
	Return 0
EndFunc

; Skill ID: 925 - $GC_I_SKILL_ID_RECALL
Func CanUse_Recall()
	Return True
EndFunc

Func BestTarget_Recall($aAggroRange)
	Return 0
EndFunc

; Skill ID: 926 - $GC_I_SKILL_ID_SHARPEN_DAGGERS
Func CanUse_SharpenDaggers()
	Return True
EndFunc

Func BestTarget_SharpenDaggers($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 930 - $GC_I_SKILL_ID_AUSPICIOUS_INCANTATION
Func CanUse_AuspiciousIncantation()
	Return True
EndFunc

Func BestTarget_AuspiciousIncantation($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 949 - $GC_I_SKILL_ID_WAY_OF_THE_FOX
Func CanUse_WayOfTheFox()
	Return True
EndFunc

Func BestTarget_WayOfTheFox($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 956 - $GC_I_SKILL_ID_ENERGY_FONT
Func CanUse_EnergyFont()
	Return True
EndFunc

Func BestTarget_EnergyFont($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 957 - $GC_I_SKILL_ID_SPELL_SHIELD
Func CanUse_SpellShield()
	Return True
EndFunc

Func BestTarget_SpellShield($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 977 - $GC_I_SKILL_ID_WAY_OF_THE_LOTUS
Func CanUse_WayOfTheLotus()
	Return True
EndFunc

Func BestTarget_WayOfTheLotus($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 984 - $GC_I_SKILL_ID_TORCH_ENCHANTMENT
Func CanUse_TorchEnchantment()
	Return True
EndFunc

Func BestTarget_TorchEnchantment($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 987 - $GC_I_SKILL_ID_WAY_OF_THE_EMPTY_PALM
Func CanUse_WayOfTheEmptyPalm()
	Return True
EndFunc

Func BestTarget_WayOfTheEmptyPalm($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1006 - $GC_I_SKILL_ID_ICE_FORT
Func CanUse_IceFort()
	Return True
EndFunc

Func BestTarget_IceFort($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1013 - $GC_I_SKILL_ID_ICE_BREAKER
Func CanUse_IceBreaker()
	Return True
EndFunc

Func BestTarget_IceBreaker($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1027 - $GC_I_SKILL_ID_CRITICAL_DEFENSES
Func CanUse_CriticalDefenses()
	Return True
EndFunc

Func BestTarget_CriticalDefenses($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1028 - $GC_I_SKILL_ID_WAY_OF_PERFECTION
Func CanUse_WayOfPerfection()
	Return True
EndFunc

Func BestTarget_WayOfPerfection($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1029 - $GC_I_SKILL_ID_DARK_APOSTASY
Func CanUse_DarkApostasy()
	Return True
EndFunc

Func BestTarget_DarkApostasy($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1030 - $GC_I_SKILL_ID_LOCUSTS_FURY
Func CanUse_LocustsFury()
	Return True
EndFunc

Func BestTarget_LocustsFury($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1031 - $GC_I_SKILL_ID_SHROUD_OF_DISTRESS
Func CanUse_ShroudOfDistress()
	Return True
EndFunc

Func BestTarget_ShroudOfDistress($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1054 - $GC_I_SKILL_ID_ANCESTORS_VISAGE
Func CanUse_AncestorsVisage()
	Return True
EndFunc

Func BestTarget_AncestorsVisage($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1084 - $GC_I_SKILL_ID_SLIVER_ARMOR
Func CanUse_SliverArmor()
	Return True
EndFunc

Func BestTarget_SliverArmor($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1091 - $GC_I_SKILL_ID_DOUBLE_DRAGON
Func CanUse_DoubleDragon()
	Return True
EndFunc

Func BestTarget_DoubleDragon($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1114 - $GC_I_SKILL_ID_SPIRIT_BOND
Func CanUse_SpiritBond()
	Return True
EndFunc

Func BestTarget_SpiritBond($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1115 - $GC_I_SKILL_ID_AIR_OF_ENCHANTMENT
Func CanUse_AirOfEnchantment()
	Return True
EndFunc

Func BestTarget_AirOfEnchantment($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1123 - $GC_I_SKILL_ID_LIFE_SHEATH
Func CanUse_LifeSheath()
	Return True
EndFunc

Func BestTarget_LifeSheath($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1152 - $GC_I_SKILL_ID_DEMONIC_AGILITY
Func CanUse_DemonicAgility()
	Return True
EndFunc

Func BestTarget_DemonicAgility($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1153 - $GC_I_SKILL_ID_BLESSING_OF_THE_KIRIN
Func CanUse_BlessingOfTheKirin()
	Return True
EndFunc

Func BestTarget_BlessingOfTheKirin($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1229 - $GC_I_SKILL_ID_EXPLOSIVE_GROWTH
Func CanUse_ExplosiveGrowth()
	Return True
EndFunc

Func BestTarget_ExplosiveGrowth($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1230 - $GC_I_SKILL_ID_BOON_OF_CREATION
Func CanUse_BoonOfCreation()
	Return True
EndFunc

Func BestTarget_BoonOfCreation($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1231 - $GC_I_SKILL_ID_SPIRIT_CHANNELING
Func CanUse_SpiritChanneling()
	Return True
EndFunc

Func BestTarget_SpiritChanneling($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1244 - $GC_I_SKILL_ID_GHOSTLY_HASTE
Func CanUse_GhostlyHaste()
	Return True
EndFunc

Func BestTarget_GhostlyHaste($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1261 - $GC_I_SKILL_ID_FRIGID_ARMOR
Func CanUse_FrigidArmor()
	Return True
EndFunc

Func BestTarget_FrigidArmor($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1311 - $GC_I_SKILL_ID_NIGHTMARE_REFUGE
Func CanUse_NightmareRefuge()
	Return True
EndFunc

Func BestTarget_NightmareRefuge($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1323 - $GC_I_SKILL_ID_SUGAR_RUSH_MEDIUM
Func CanUse_SugarRushMedium()
	Return True
EndFunc

Func BestTarget_SugarRushMedium($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1338 - $GC_I_SKILL_ID_PERSISTENCE_OF_MEMORY
Func CanUse_PersistenceOfMemory()
	Return True
EndFunc

Func BestTarget_PersistenceOfMemory($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1340 - $GC_I_SKILL_ID_SYMBOLIC_CELERITY
Func CanUse_SymbolicCelerity()
	Return True
EndFunc

Func BestTarget_SymbolicCelerity($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1355 - $GC_I_SKILL_ID_JAGGED_BONES
Func CanUse_JaggedBones()
	Return True
EndFunc

Func BestTarget_JaggedBones($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1356 - $GC_I_SKILL_ID_CONTAGION
Func CanUse_Contagion()
	Return True
EndFunc

Func BestTarget_Contagion($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1357 - $GC_I_SKILL_ID_BLOODLETTING
Func CanUse_Bloodletting()
	Return True
EndFunc

Func BestTarget_Bloodletting($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1370 - $GC_I_SKILL_ID_STORM_DJINNS_HASTE
Func CanUse_StormDjinnsHaste()
	Return True
EndFunc

Func BestTarget_StormDjinnsHaste($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1371 - $GC_I_SKILL_ID_STONE_STRIKER
Func CanUse_StoneStriker()
	Return True
EndFunc

Func BestTarget_StoneStriker($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1373 - $GC_I_SKILL_ID_STONE_SHEATH
Func CanUse_StoneSheath()
	Return True
EndFunc

Func BestTarget_StoneSheath($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1375 - $GC_I_SKILL_ID_STONEFLESH_AURA
Func CanUse_StonefleshAura()
	Return True
EndFunc

Func BestTarget_StonefleshAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1378 - $GC_I_SKILL_ID_MASTER_OF_MAGIC
Func CanUse_MasterOfMagic()
	Return True
EndFunc

Func BestTarget_MasterOfMagic($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1381 - $GC_I_SKILL_ID_FLAME_DJINNS_HASTE1
Func CanUse_FlameDjinnsHaste1()
	Return True
EndFunc

Func BestTarget_FlameDjinnsHaste1($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1390 - $GC_I_SKILL_ID_JUDGES_INTERVENTION
Func CanUse_JudgesIntervention()
	Return True
EndFunc

Func BestTarget_JudgesIntervention($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1391 - $GC_I_SKILL_ID_SUPPORTIVE_SPIRIT
Func CanUse_SupportiveSpirit()
	Return True
EndFunc

Func BestTarget_SupportiveSpirit($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1392 - $GC_I_SKILL_ID_WATCHFUL_HEALING
Func CanUse_WatchfulHealing()
	Return True
EndFunc

Func BestTarget_WatchfulHealing($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1393 - $GC_I_SKILL_ID_HEALERS_BOON
Func CanUse_HealersBoon()
	Return True
EndFunc

Func BestTarget_HealersBoon($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1394 - $GC_I_SKILL_ID_HEALERS_COVENANT
Func CanUse_HealersCovenant()
	Return True
EndFunc

Func BestTarget_HealersCovenant($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1395 - $GC_I_SKILL_ID_BALTHAZARS_PENDULUM
Func CanUse_BalthazarsPendulum()
	Return True
EndFunc

Func BestTarget_BalthazarsPendulum($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1399 - $GC_I_SKILL_ID_SHIELD_OF_ABSORPTION
Func CanUse_ShieldOfAbsorption()
	Return True
EndFunc

Func BestTarget_ShieldOfAbsorption($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1400 - $GC_I_SKILL_ID_REVERSAL_OF_DAMAGE
Func CanUse_ReversalOfDamage()
	Return True
EndFunc

Func BestTarget_ReversalOfDamage($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1450 - $GC_I_SKILL_ID_ABADDONS_CONSPIRACY
Func CanUse_AbaddonsConspiracy()
	Return True
EndFunc

Func BestTarget_AbaddonsConspiracy($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1457 - $GC_I_SKILL_ID_ABADDONS_CHOSEN
Func CanUse_AbaddonsChosen()
	Return True
EndFunc

Func BestTarget_AbaddonsChosen($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1480 - $GC_I_SKILL_ID_SPIRITS_GIFT
Func CanUse_SpiritsGift()
	Return True
EndFunc

Func BestTarget_SpiritsGift($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1492 - $GC_I_SKILL_ID_REMOVE_WIND_PRAYERS_SKILL
Func CanUse_RemoveWindPrayersSkill()
	Return True
EndFunc

Func BestTarget_RemoveWindPrayersSkill($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1493 - $GC_I_SKILL_ID_GRENTHS_FINGERS
Func CanUse_GrenthsFingers()
	Return True
EndFunc

Func BestTarget_GrenthsFingers($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1494 - $GC_I_SKILL_ID_REMOVE_BOON_OF_THE_GODS
Func CanUse_RemoveBoonOfTheGods()
	Return True
EndFunc

Func BestTarget_RemoveBoonOfTheGods($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1495 - $GC_I_SKILL_ID_AURA_OF_THORNS
Func CanUse_AuraOfThorns()
	Return True
EndFunc

Func BestTarget_AuraOfThorns($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1496 - $GC_I_SKILL_ID_BALTHAZARS_RAGE
Func CanUse_BalthazarsRage()
	Return True
EndFunc

Func BestTarget_BalthazarsRage($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1497 - $GC_I_SKILL_ID_DUST_CLOAK
Func CanUse_DustCloak()
	Return True
EndFunc

Func BestTarget_DustCloak($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1498 - $GC_I_SKILL_ID_STAGGERING_FORCE
Func CanUse_StaggeringForce()
	Return True
EndFunc

Func BestTarget_StaggeringForce($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1499 - $GC_I_SKILL_ID_PIOUS_RENEWAL
Func CanUse_PiousRenewal()
	Return True
EndFunc

Func BestTarget_PiousRenewal($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1500 - $GC_I_SKILL_ID_MIRAGE_CLOAK
Func CanUse_MirageCloak()
	Return True
EndFunc

Func BestTarget_MirageCloak($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1501 - $GC_I_SKILL_ID_REMOVE_BALTHAZARS_RAGE
Func CanUse_RemoveBalthazarsRage()
	Return True
EndFunc

Func BestTarget_RemoveBalthazarsRage($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1502 - $GC_I_SKILL_ID_ARCANE_ZEAL
Func CanUse_ArcaneZeal()
	Return True
EndFunc

Func BestTarget_ArcaneZeal($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1503 - $GC_I_SKILL_ID_MYSTIC_VIGOR
Func CanUse_MysticVigor()
	Return True
EndFunc

Func BestTarget_MysticVigor($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1504 - $GC_I_SKILL_ID_WATCHFUL_INTERVENTION
Func CanUse_WatchfulIntervention()
	Return True
EndFunc

Func BestTarget_WatchfulIntervention($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1505 - $GC_I_SKILL_ID_VOW_OF_PIETY
Func CanUse_VowOfPiety()
	Return True
EndFunc

Func BestTarget_VowOfPiety($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1506 - $GC_I_SKILL_ID_VITAL_BOON
Func CanUse_VitalBoon()
	Return True
EndFunc

Func BestTarget_VitalBoon($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1507 - $GC_I_SKILL_ID_HEART_OF_HOLY_FLAME
Func CanUse_HeartOfHolyFlame()
	Return True
EndFunc

Func BestTarget_HeartOfHolyFlame($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1509 - $GC_I_SKILL_ID_FAITHFUL_INTERVENTION
Func CanUse_FaithfulIntervention()
	Return True
EndFunc

Func BestTarget_FaithfulIntervention($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1510 - $GC_I_SKILL_ID_SAND_SHARDS
Func CanUse_SandShards()
	Return True
EndFunc

Func BestTarget_SandShards($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1511 - $GC_I_SKILL_ID_INTIMIDATING_AURA_BETA_VERSION
Func CanUse_IntimidatingAuraBetaVersion()
	Return True
EndFunc

Func BestTarget_IntimidatingAuraBetaVersion($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1512 - $GC_I_SKILL_ID_LYSSAS_HASTE
Func CanUse_LyssasHaste()
	Return True
EndFunc

Func BestTarget_LyssasHaste($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1513 - $GC_I_SKILL_ID_GUIDING_HANDS
Func CanUse_GuidingHands()
	Return True
EndFunc

Func BestTarget_GuidingHands($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1514 - $GC_I_SKILL_ID_FLEETING_STABILITY
Func CanUse_FleetingStability()
	Return True
EndFunc

Func BestTarget_FleetingStability($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1515 - $GC_I_SKILL_ID_ARMOR_OF_SANCTITY
Func CanUse_ArmorOfSanctity()
	Return True
EndFunc

Func BestTarget_ArmorOfSanctity($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1516 - $GC_I_SKILL_ID_MYSTIC_REGENERATION
Func CanUse_MysticRegeneration()
	Return True
EndFunc

Func BestTarget_MysticRegeneration($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1517 - $GC_I_SKILL_ID_VOW_OF_SILENCE
Func CanUse_VowOfSilence()
	Return True
EndFunc

Func BestTarget_VowOfSilence($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1523 - $GC_I_SKILL_ID_MEDITATION
Func CanUse_Meditation()
	Return True
EndFunc

Func BestTarget_Meditation($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1524 - $GC_I_SKILL_ID_EREMITES_ZEAL
Func CanUse_EremitesZeal()
	Return True
EndFunc

Func BestTarget_EremitesZeal($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1531 - $GC_I_SKILL_ID_INTIMIDATING_AURA
Func CanUse_IntimidatingAura()
	Return True
EndFunc

Func BestTarget_IntimidatingAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1540 - $GC_I_SKILL_ID_CONVICTION
Func CanUse_Conviction()
	Return True
EndFunc

Func BestTarget_Conviction($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1541 - $GC_I_SKILL_ID_ENCHANTED_HASTE
Func CanUse_EnchantedHaste()
	Return True
EndFunc

Func BestTarget_EnchantedHaste($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1544 - $GC_I_SKILL_ID_WHIRLING_CHARGE
Func CanUse_WhirlingCharge()
	Return True
EndFunc

Func BestTarget_WhirlingCharge($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1612 - $GC_I_SKILL_ID_SUGAR_RUSH_LONG
Func CanUse_SugarRushLong()
	Return True
EndFunc

Func BestTarget_SugarRushLong($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1638 - $GC_I_SKILL_ID_DEADLY_HASTE
Func CanUse_DeadlyHaste()
	Return True
EndFunc

Func BestTarget_DeadlyHaste($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1639 - $GC_I_SKILL_ID_ASSASSINS_REMEDY
Func CanUse_AssassinsRemedy()
	Return True
EndFunc

Func BestTarget_AssassinsRemedy($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1640 - $GC_I_SKILL_ID_FOXS_PROMISE
Func CanUse_FoxsPromise()
	Return True
EndFunc

Func BestTarget_FoxsPromise($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1641 - $GC_I_SKILL_ID_FEIGNED_NEUTRALITY
Func CanUse_FeignedNeutrality()
	Return True
EndFunc

Func BestTarget_FeignedNeutrality($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1654 - $GC_I_SKILL_ID_SHADOW_MELD
Func CanUse_ShadowMeld()
	Return True
EndFunc

Func BestTarget_ShadowMeld($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1663 - $GC_I_SKILL_ID_ELEMENTAL_FLAME
Func CanUse_ElementalFlame()
	Return True
EndFunc

Func BestTarget_ElementalFlame($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1683 - $GC_I_SKILL_ID_PENSIVE_GUARDIAN
Func CanUse_PensiveGuardian()
	Return True
EndFunc

Func BestTarget_PensiveGuardian($aAggroRange)
	Return 0
EndFunc

; Skill ID: 1684 - $GC_I_SKILL_ID_SCRIBES_INSIGHT
Func CanUse_ScribesInsight()
	Return True
EndFunc

Func BestTarget_ScribesInsight($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1685 - $GC_I_SKILL_ID_HOLY_HASTE
Func CanUse_HolyHaste()
	Return True
EndFunc

Func BestTarget_HolyHaste($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1736 - $GC_I_SKILL_ID_SPIRITS_STRENGTH
Func CanUse_SpiritsStrength()
	Return True
EndFunc

Func BestTarget_SpiritsStrength($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1737 - $GC_I_SKILL_ID_WIELDERS_ZEAL
Func CanUse_WieldersZeal()
	Return True
EndFunc

Func BestTarget_WieldersZeal($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1738 - $GC_I_SKILL_ID_SIGHT_BEYOND_SIGHT
Func CanUse_SightBeyondSight()
	Return True
EndFunc

Func BestTarget_SightBeyondSight($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1739 - $GC_I_SKILL_ID_RENEWING_MEMORIES
Func CanUse_RenewingMemories()
	Return True
EndFunc

Func BestTarget_RenewingMemories($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1740 - $GC_I_SKILL_ID_WIELDERS_REMEDY
Func CanUse_WieldersRemedy()
	Return True
EndFunc

Func BestTarget_WieldersRemedy($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1754 - $GC_I_SKILL_ID_ONSLAUGHT
Func CanUse_Onslaught()
	Return True
EndFunc

Func BestTarget_Onslaught($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1755 - $GC_I_SKILL_ID_MYSTIC_CORRUPTION
Func CanUse_MysticCorruption()
	Return True
EndFunc

Func BestTarget_MysticCorruption($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1756 - $GC_I_SKILL_ID_GRENTHS_GRASP
Func CanUse_GrenthsGrasp()
	Return True
EndFunc

Func BestTarget_GrenthsGrasp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1757 - $GC_I_SKILL_ID_VEIL_OF_THORNS
Func CanUse_VeilOfThorns()
	Return True
EndFunc

Func BestTarget_VeilOfThorns($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1758 - $GC_I_SKILL_ID_HARRIERS_GRASP
Func CanUse_HarriersGrasp()
	Return True
EndFunc

Func BestTarget_HarriersGrasp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1759 - $GC_I_SKILL_ID_VOW_OF_STRENGTH
Func CanUse_VowOfStrength()
	Return True
EndFunc

Func BestTarget_VowOfStrength($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1760 - $GC_I_SKILL_ID_EBON_DUST_AURA
Func CanUse_EbonDustAura()
	Return True
EndFunc

Func BestTarget_EbonDustAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1761 - $GC_I_SKILL_ID_ZEALOUS_VOW
Func CanUse_ZealousVow()
	Return True
EndFunc

Func BestTarget_ZealousVow($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1763 - $GC_I_SKILL_ID_ZEALOUS_RENEWAL
Func CanUse_ZealousRenewal()
	Return True
EndFunc

Func BestTarget_ZealousRenewal($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1764 - $GC_I_SKILL_ID_ATTACKERS_INSIGHT
Func CanUse_AttackersInsight()
	Return True
EndFunc

Func BestTarget_AttackersInsight($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1765 - $GC_I_SKILL_ID_RENDING_AURA
Func CanUse_RendingAura()
	Return True
EndFunc

Func BestTarget_RendingAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1766 - $GC_I_SKILL_ID_FEATHERFOOT_GRACE
Func CanUse_FeatherfootGrace()
	Return True
EndFunc

Func BestTarget_FeatherfootGrace($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1768 - $GC_I_SKILL_ID_HARRIERS_HASTE
Func CanUse_HarriersHaste()
	Return True
EndFunc

Func BestTarget_HarriersHaste($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1787 - $GC_I_SKILL_ID_ACCELERATED_GROWTH
Func CanUse_AcceleratedGrowth()
	Return True
EndFunc

Func BestTarget_AcceleratedGrowth($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1820 - $GC_I_SKILL_ID_SPIRIT_FORM_REMAINS_OF_SAHLAHJA
Func CanUse_SpiritFormRemainsOfSahlahja()
	Return True
EndFunc

Func BestTarget_SpiritFormRemainsOfSahlahja($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1821 - $GC_I_SKILL_ID_GODS_BLESSING
Func CanUse_GodsBlessing()
	Return True
EndFunc

Func BestTarget_GodsBlessing($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1860 - $GC_I_SKILL_ID_SUGAR_RUSH_SHORT
Func CanUse_SugarRushShort()
	Return True
EndFunc

Func BestTarget_SugarRushShort($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1916 - $GC_I_SKILL_ID_SUGAR_JOLT_SHORT
Func CanUse_SugarJoltShort()
	Return True
EndFunc

Func BestTarget_SugarJoltShort($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1933 - $GC_I_SKILL_ID_SUGAR_JOLT_LONG
Func CanUse_SugarJoltLong()
	Return True
EndFunc

Func BestTarget_SugarJoltLong($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1948 - $GC_I_SKILL_ID_SHADOW_SANCTUARY_LUXON2
Func CanUse_ShadowSanctuaryLuxon2()
	Return True
EndFunc

Func BestTarget_ShadowSanctuaryLuxon2($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1951 - $GC_I_SKILL_ID_ELEMENTAL_LORD_LUXON
Func CanUse_ElementalLordLuxon()
	Return True
EndFunc

Func BestTarget_ElementalLordLuxon($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1952 - $GC_I_SKILL_ID_SELFLESS_SPIRIT_LUXON
Func CanUse_SelflessSpiritLuxon()
	Return True
EndFunc

Func BestTarget_SelflessSpiritLuxon($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1955 - $GC_I_SKILL_ID_AURA_OF_HOLY_MIGHT_LUXON
Func CanUse_AuraOfHolyMightLuxon()
	Return True
EndFunc

Func BestTarget_AuraOfHolyMightLuxon($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1989 - $GC_I_SKILL_ID_WAY_OF_THE_MANTIS
Func CanUse_WayOfTheMantis()
	Return True
EndFunc

Func BestTarget_WayOfTheMantis($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 1997 - $GC_I_SKILL_ID_WITHERING_AURA
Func CanUse_WitheringAura()
	Return True
EndFunc

Func BestTarget_WitheringAura($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2005 - $GC_I_SKILL_ID_SMITERS_BOON
Func CanUse_SmitersBoon()
	Return True
EndFunc

Func BestTarget_SmitersBoon($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2007 - $GC_I_SKILL_ID_PURIFYING_VEIL
Func CanUse_PurifyingVeil()
	Return True
EndFunc

Func BestTarget_PurifyingVeil($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2013 - $GC_I_SKILL_ID_GRENTHS_AURA
Func CanUse_GrenthsAura()
	Return True
EndFunc

Func BestTarget_GrenthsAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2061 - $GC_I_SKILL_ID_PATIENT_SPIRIT
Func CanUse_PatientSpirit()
	Return True
EndFunc

Func BestTarget_PatientSpirit($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2063 - $GC_I_SKILL_ID_AURA_OF_STABILITY
Func CanUse_AuraOfStability()
	Return True
EndFunc

Func BestTarget_AuraOfStability($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2064 - $GC_I_SKILL_ID_SPOTLESS_MIND
Func CanUse_SpotlessMind()
	Return True
EndFunc

Func BestTarget_SpotlessMind($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2065 - $GC_I_SKILL_ID_SPOTLESS_SOUL
Func CanUse_SpotlessSoul()
	Return True
EndFunc

Func BestTarget_SpotlessSoul($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2091 - $GC_I_SKILL_ID_SHADOW_SANCTUARY_KURZICK
Func CanUse_ShadowSanctuaryKurzick()
	Return True
EndFunc

Func BestTarget_ShadowSanctuaryKurzick($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2094 - $GC_I_SKILL_ID_ELEMENTAL_LORD_KURZICK
Func CanUse_ElementalLordKurzick()
	Return True
EndFunc

Func BestTarget_ElementalLordKurzick($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2095 - $GC_I_SKILL_ID_SELFLESS_SPIRIT_KURZICK
Func CanUse_SelflessSpiritKurzick()
	Return True
EndFunc

Func BestTarget_SelflessSpiritKurzick($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2098 - $GC_I_SKILL_ID_AURA_OF_HOLY_MIGHT_KURZICK
Func CanUse_AuraOfHolyMightKurzick()
	Return True
EndFunc

Func BestTarget_AuraOfHolyMightKurzick($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2101 - $GC_I_SKILL_ID_CRITICAL_AGILITY
Func CanUse_CriticalAgility()
	Return True
EndFunc

Func BestTarget_CriticalAgility($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2105 - $GC_I_SKILL_ID_SEED_OF_LIFE
Func CanUse_SeedOfLife()
	Return True
EndFunc

Func BestTarget_SeedOfLife($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2109 - $GC_I_SKILL_ID_ETERNAL_AURA
Func CanUse_EternalAura()
	If CachedAgent_HasEffect($GC_I_SKILL_ID_CURSE_OF_DHUUM) Or CachedAgent_HasEffect($GC_I_SKILL_ID_FROZEN_SOIL) Then Return False
	Return True
EndFunc

Func BestTarget_EternalAura($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2128 - $GC_I_SKILL_ID_VOLFEN_POUNCE_CURSE_OF_THE_NORNBEAR
Func CanUse_VolfenPounceCurseOfTheNornbear()
	Return True
EndFunc

Func BestTarget_VolfenPounceCurseOfTheNornbear($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2138 - $GC_I_SKILL_ID_HEXERS_VIGOR
Func CanUse_HexersVigor()
	Return True
EndFunc

Func BestTarget_HexersVigor($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2139 - $GC_I_SKILL_ID_MASOCHISM
Func CanUse_Masochism()
	Return True
EndFunc

Func BestTarget_Masochism($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2187 - $GC_I_SKILL_ID_WAY_OF_THE_MASTER
Func CanUse_WayOfTheMaster()
	Return True
EndFunc

Func BestTarget_WayOfTheMaster($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2190 - $GC_I_SKILL_ID_MAGNETIC_SURGE
Func CanUse_MagneticSurge()
	Return True
EndFunc

Func BestTarget_MagneticSurge($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2201 - $GC_I_SKILL_ID_SHIELD_OF_FORCE
Func CanUse_ShieldOfForce()
	Return True
EndFunc

Func BestTarget_ShieldOfForce($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2220 - $GC_I_SKILL_ID_GREAT_DWARF_ARMOR
Func CanUse_GreatDwarfArmor()
	Return True
EndFunc

Func BestTarget_GreatDwarfArmor($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2249 - $GC_I_SKILL_ID_POLYMOCK_BLOCK
Func CanUse_PolymockBlock()
	Return True
EndFunc

Func BestTarget_PolymockBlock($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2265 - $GC_I_SKILL_ID_POLYMOCK_LIGHTNING_DJINNS_HASTE
Func CanUse_PolymockLightningDjinnsHaste()
	Return True
EndFunc

Func BestTarget_PolymockLightningDjinnsHaste($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2280 - $GC_I_SKILL_ID_POLYMOCK_FROZEN_ARMOR
Func CanUse_PolymockFrozenArmor()
	Return True
EndFunc

Func BestTarget_PolymockFrozenArmor($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2328 - $GC_I_SKILL_ID_CRYSTAL_SHIELD
Func CanUse_CrystalShield()
	Return True
EndFunc

Func BestTarget_CrystalShield($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2383 - $GC_I_SKILL_ID_VOLFEN_AGILITY
Func CanUse_VolfenAgility()
	Return True
EndFunc

Func BestTarget_VolfenAgility($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2411 - $GC_I_SKILL_ID_MINDBENDER
Func CanUse_Mindbender()
	Return True
EndFunc

Func BestTarget_Mindbender($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2417 - $GC_I_SKILL_ID_MENTAL_BLOCK
Func CanUse_MentalBlock()
	Return True
EndFunc

Func BestTarget_MentalBlock($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2423 - $GC_I_SKILL_ID_DWARVEN_STABILITY
Func CanUse_DwarvenStability()
	Return True
EndFunc

Func BestTarget_DwarvenStability($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2635 - $GC_I_SKILL_ID_FLAME_DJINNS_HASTE2
Func CanUse_FlameDjinnsHaste2()
	Return True
EndFunc

Func BestTarget_FlameDjinnsHaste2($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2638 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2655 - $GC_I_SKILL_ID_DRAGON_EMPIRE_RAGE
Func CanUse_DragonEmpireRage()
	Return True
EndFunc

Func BestTarget_DragonEmpireRage($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2730 - $GC_I_SKILL_ID_AURA_OF_PURITY
Func CanUse_AuraOfPurity()
	Return True
EndFunc

Func BestTarget_AuraOfPurity($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2791 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2797 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2805 - $GC_I_SKILL_ID_MIST_FORM_PVP
Func CanUse_MistFormPvp()
	Return True
EndFunc

Func BestTarget_MistFormPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2857 - $GC_I_SKILL_ID_AEGIS_PVP
Func CanUse_AegisPvp()
	Return True
EndFunc

Func BestTarget_AegisPvp($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2860 - $GC_I_SKILL_ID_ETHER_RENEWAL_PVP
Func CanUse_EtherRenewalPvp()
	Return True
EndFunc

Func BestTarget_EtherRenewalPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2862 - $GC_I_SKILL_ID_SHADOW_FORM_PVP
Func CanUse_ShadowFormPvp()
	Return True
EndFunc

Func BestTarget_ShadowFormPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2869 - $GC_I_SKILL_ID_ASSASSINS_REMEDY_PVP
Func CanUse_AssassinsRemedyPvp()
	Return True
EndFunc

Func BestTarget_AssassinsRemedyPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2884 - $GC_I_SKILL_ID_MYSTIC_REGENERATION_PVP
Func CanUse_MysticRegenerationPvp()
	Return True
EndFunc

Func BestTarget_MysticRegenerationPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2891 - $GC_I_SKILL_ID_UNYIELDING_AURA_PVP
Func CanUse_UnyieldingAuraPvp()
	Return True
EndFunc

Func BestTarget_UnyieldingAuraPvp($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2892 - $GC_I_SKILL_ID_SPIRIT_BOND_PVP
Func CanUse_SpiritBondPvp()
	Return True
EndFunc

Func BestTarget_SpiritBondPvp($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2895 - $GC_I_SKILL_ID_SMITERS_BOON_PVP
Func CanUse_SmitersBoonPvp()
	Return True
EndFunc

Func BestTarget_SmitersBoonPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 2912 - $GC_I_SKILL_ID_BIT_GOLEM_RECTIFIER
Func CanUse_BitGolemRectifier()
	Return True
EndFunc

Func BestTarget_BitGolemRectifier($aAggroRange)
	Return 0
EndFunc

; Skill ID: 2915 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2934 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2943 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2956 - ;  $GC_I_SKILL_ID_UNKNOWN
; Skill ID: 2999 - $GC_I_SKILL_ID_STRENGTH_OF_HONOR_PVP
Func CanUse_StrengthOfHonorPvp()
	Return True
EndFunc

Func BestTarget_StrengthOfHonorPvp($aAggroRange)
	Return 0
EndFunc

; Skill ID: 3003 - $GC_I_SKILL_ID_ARMOR_OF_UNFEELING_PVP
Func CanUse_ArmorOfUnfeelingPvp()
	Return True
EndFunc

Func BestTarget_ArmorOfUnfeelingPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3048 - $GC_I_SKILL_ID_SHROUD_OF_DISTRESS_PVP
Func CanUse_ShroudOfDistressPvp()
	Return True
EndFunc

Func BestTarget_ShroudOfDistressPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3054 - $GC_I_SKILL_ID_MASOCHISM_PVP
Func CanUse_MasochismPvp()
	Return True
EndFunc

Func BestTarget_MasochismPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3181 - $GC_I_SKILL_ID_ILLUSIONARY_WEAPONRY_PVP
Func CanUse_IllusionaryWeaponryPvp()
	Return True
EndFunc

Func BestTarget_IllusionaryWeaponryPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3267 - $GC_I_SKILL_ID_EBON_DUST_AURA_PVP
Func CanUse_EbonDustAuraPvp()
	Return True
EndFunc

Func BestTarget_EbonDustAuraPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3268 - $GC_I_SKILL_ID_HEART_OF_HOLY_FLAME_PVP
Func CanUse_HeartOfHolyFlamePvp()
	Return True
EndFunc

Func BestTarget_HeartOfHolyFlamePvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3269 - $GC_I_SKILL_ID_GUIDING_HANDS_PVP
Func CanUse_GuidingHandsPvp()
	Return True
EndFunc

Func BestTarget_GuidingHandsPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3346 - $GC_I_SKILL_ID_AURA_OF_THORNS_PVP
Func CanUse_AuraOfThornsPvp()
	Return True
EndFunc

Func BestTarget_AuraOfThornsPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3347 - $GC_I_SKILL_ID_DUST_CLOAK_PVP
Func CanUse_DustCloakPvp()
	Return True
EndFunc

Func BestTarget_DustCloakPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3348 - $GC_I_SKILL_ID_LYSSAS_HASTE_PVP
Func CanUse_LyssasHastePvp()
	Return True
EndFunc

Func BestTarget_LyssasHastePvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3365 - $GC_I_SKILL_ID_ONSLAUGHT_PVP
Func CanUse_OnslaughtPvp()
	Return True
EndFunc

Func BestTarget_OnslaughtPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3373 - $GC_I_SKILL_ID_ILLUSION_OF_HASTE_PVP
Func CanUse_IllusionOfHastePvp()
	Return True
EndFunc

Func BestTarget_IllusionOfHastePvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3375 - $GC_I_SKILL_ID_AURA_OF_RESTORATION_PVP
Func CanUse_AuraOfRestorationPvp()
	Return True
EndFunc

Func BestTarget_AuraOfRestorationPvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3397 - $GC_I_SKILL_ID_ELEMENTAL_FLAME_PVP
Func CanUse_ElementalFlamePvp()
	Return True
EndFunc

Func BestTarget_ElementalFlamePvp($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3423 - $GC_I_SKILL_ID_SOUL_TAKER
Func CanUse_SoulTaker()
	Return True
EndFunc

Func BestTarget_SoulTaker($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3424 - $GC_I_SKILL_ID_OVER_THE_LIMIT
Func CanUse_OverTheLimit()
	Return True
EndFunc

Func BestTarget_OverTheLimit($aAggroRange)
	Return Agent_GetMyID()
EndFunc

; Skill ID: 3430 - $GC_I_SKILL_ID_VOW_OF_REVOLUTION
Func CanUse_VowOfRevolution()
	Return True
EndFunc

Func BestTarget_VowOfRevolution($aAggroRange)
	Return Agent_GetMyID()
EndFunc

